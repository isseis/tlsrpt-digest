...
1. Run `git --no-pager diff --staged`. For a message-only amend, check if a
   commit exists, then run `git log -1` to inspect it.
...
5. If the user confirms, run `git commit -m "<message>"` for a new commit
   or `git commit --amend -m "<message>"` for an amend.
