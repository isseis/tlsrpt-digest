# 要件定義書：設定ファイル読み込み（TOML）

## ドキュメントステータス

| 項目 | 内容 |
|---|---|
| ステータス | `draft` |
| 作成日 | 2026-05-12 |
| レビュー日 | 2026-05-12 |
| レビュアー | isseis |
| コメント | - |

---

## 1. 背景と目的

### 1.1 背景

tlsrpt-digest は IMAP 接続情報、通知先など複数の設定項目を持つ。
これらを TOML 形式の設定ファイルから読み込む機能を実装する。

### 1.2 目的

1. **主目的**: TOML 設定ファイルを読み込み、アプリケーション設定を構造体として提供する
2. **副次的目的**: 設定値のバリデーションにより早期に設定ミスを検出する

---

## 2. スコープ

### 対象範囲（In Scope）

- TOML 設定ファイルの読み込みと Go 構造体へのデコード
- 必須設定項目の存在チェック
- 設定値のバリデーション（型・範囲・形式）
- コマンドライン引数による設定ファイルパスの指定

### 対象外（Out of Scope）

- 設定ファイルのホットリロード（アプリ再起動で反映）
- 環境変数による設定上書き（将来の拡張）
- 設定ファイルの暗号化

### 影響を受けるコンポーネント

- **直接変更**: `internal/config/`（新規作成）または `cmd/tlsrpt-digest/` 内
- **間接的影響**: `cmd/tlsrpt-digest/`（設定の利用側）

---

## 3. 機能要件

### F-001: 設定ファイルの読み込み

指定されたパスの TOML ファイルを読み込み、`Config` 構造体にデコードする。

**受け入れ条件（Acceptance Criteria）**:

- `AC-01`: 有効な TOML ファイルを指定した場合、すべての設定値を正しく読み込む
- `AC-02`: 指定ファイルが存在しない場合、エラーを返す
- `AC-03`: TOML の文法エラーがある場合、エラーを返す
- `AC-04`: 未知のキーが含まれる場合、エラーまたは警告を出力する（strict モード）

### F-002: 設定値のバリデーション

読み込んだ設定値が有効であることを検証する。

**受け入れ条件（Acceptance Criteria）**:

- `AC-05`: IMAP ホスト名が空の場合はエラーを返す
- `AC-06`: IMAP ポート番号が 1〜65535 の範囲外の場合はエラーを返す
- `AC-07`: IMAP ユーザ名またはパスワードが空の場合はエラーを返す
- `AC-08`: Slack Webhook URL は環境変数（`TLSRPT_SLACK_WEBHOOK_URL_SUCCESS`・`TLSRPT_SLACK_WEBHOOK_URL_ERROR`）で管理するため、設定ファイルには `notify.slack.allowed_host` のみを記載する。「少なくとも1つの通知手段が設定されているか」の検証は `internal/notify` パッケージが担い、`internal/config` は `notify.slack.allowed_host` の形式チェック（ポート番号・スキームを含まない完全修飾ホスト名）のみを行う
- `AC-09`: `imap.fetch_days` が 1 未満の場合はエラーを返す
- `AC-10`: `imap.tls_ca_cert` が設定されている場合、指定パスのファイルが存在しかつ PEM 形式の証明書として読み込めることを確認する。読み込めない場合はエラーを返す

### F-003: デフォルト値の適用

省略可能な設定項目にはデフォルト値を適用する。

**受け入れ条件（Acceptance Criteria）**:

- `AC-11`: IMAP メールボックス名が未設定の場合、デフォルト値（`"INBOX"`）が適用される。TLSRPT レポートをサーバ側フィルタで専用フォルダ（例：`tls-reports`）に振り分けている場合は明示的に指定する
- `AC-12`: `imap.fetch_days` が未設定の場合、デフォルト値（`14`）が適用される
- `AC-13`: ストレージルートディレクトリ（`store.root_dir`）が未設定の場合、デフォルト値（例：`"./store"`）が適用される。データファイルパス（`{root_dir}/tlsrpt.json`）およびメール保存ディレクトリ（`{root_dir}/emails/`）はプログラムが自動的に導出するため、個別に設定することはできない
- `AC-14`: `imap.tls_ca_cert` が未設定の場合、OS のシステム CA バンドルを使用する（デフォルト動作）
- `AC-15`: 定期サマリの集計期間（`summary.window_days`）が未設定の場合、デフォルト値（`7`）が適用される。`summary` サブコマンドの `--since` フラグで上書き可能
- `AC-16`: レポートレコードの保持期間（`store.retention_days`）が未設定の場合、デフォルト値（例：`30`）が適用される。`gc` サブコマンドの `--before` フラグで上書き可能
- `AC-17`: メール保存ディレクトリの最大保持期間（`store.max_email_age_days`）が未設定の場合、デフォルト値（例：`30`）が適用される。`gc` サブコマンドの `--max-email-age` フラグで上書き可能。TOML キー名と既定値の詳細は `02_architecture.md` で確定する

---

## 4. 非機能要件

### セキュリティ

- パスワードや Webhook URL が設定ファイルのパースエラーメッセージに含まれないこと

### 保守性

- `Config` 構造体は各パッケージ（imap、notify、store）の設定をネストした形で持つ

---

## 5. 制約

- 使用言語は Go とする（Go 1.26 以上）
- TOML ライブラリには `BurntSushi/toml` を使用する
- テストには `stretchr/testify` を使用する

---

## 6. テスト方針

### 単体テスト

- 有効な設定ファイルの読み込みテスト
- 必須項目欠如時のバリデーションエラーテスト
- デフォルト値適用のテスト（`imap.tls_ca_cert` 未設定でシステム CA が使われること）
- `imap.tls_ca_cert` に存在しないパスや不正なファイルを指定した場合のエラーテスト
- 不正 TOML（文法エラー）のエラーテスト
