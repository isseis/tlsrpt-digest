...
- [ ] **2.4** `internal/config/helpers_test.go` を新規作成し `LoadFile` テスト向けログキャプチャヘルパーを定義する
  ...
  - 作業内容:
    - `//go:build test` は付けない。`_test.go` ファイルはビルドタグなしで `go test ./internal/config/` に含まれる。ビルドタグを付けると `load_file_test.go` がこのヘルパーに依存したとき `-tags test` なしの `go test` でコンパイルエラーになる。`cmd/tlsrpt-digest/main_test.go` のタグはそのパッケージ固有の事情によるものであり、`internal/config` には転用しない
