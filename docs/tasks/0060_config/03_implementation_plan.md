...
- [ ] **2.4** `internal/config/helpers_test.go` を新規作成し `LoadFile` テスト向けログキャプチャヘルパーを定義する
  - ファイル: `internal/config/helpers_test.go`（新規、`//go:build test`、`package config_test`）
  - 作業内容:
    - ファイル先頭に `//go:build test` を記述する（既存の `cmd/tlsrpt-digest/main_test.go` と同じプロジェクト規約に準拠）
