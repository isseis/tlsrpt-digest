...
MIME bomb 攻撃（深くネストされた `multipart/*` 構造による CPU/メモリ消費）を防ぐため、再帰的なパースの深さに上限を設ける。TLSRPT レポートアドレスは DNS TXT レコードで公開されるため、任意のホストが細工したメールを送信できる。

**受け入れ条件（Acceptance Criteria）**:

- **AC-17**: `multipart/*` のネスト深度が上限（デフォルト 10）を超えた場合、エラーを返す。上限値はパッケージ内定数とし、呼び出し元からの変更は不要とする
...
- F-003 のネスト深度上限により、MIME bomb による CPU/スタック消費を防ぐ
...
- MIME ネスト深度が上限を超えた場合のエラーテスト（AC-17）
