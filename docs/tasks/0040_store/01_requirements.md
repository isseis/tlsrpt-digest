...
- `AC-08c`: `SaveEmailMetas` でメールインデックスに `{uid, uidvalidity, sent_at, saved_at}` エントリをまとめて登録できる（バッチ操作・1 回のアトミック書き込み）。`sent_at` は IMAP ENVELOPE の Date フィールド由来の送信日時（RFC 2822 Date ヘッダー、RFC 3339 へ正規化）を保持し、`{YYYYMM}` パス再構築と GC 時の対象特定に使用する。fetch サイクルで全 `.eml` を保存した後、1 回だけ呼び出すことで O(N²) の JSON 読み書きを回避する
- `AC-08d`: `SaveEmailMetas` は同一 `{uid, uidvalidity}` の既存エントリには影響しない（`saved_at` の上書きや `report_end_date` のリセットを行わない、冪等動作）。これにより、呼び出し側がローカル `.eml` のある全 UID を毎回渡すことで、過去 fetch サイクルで `SaveEmailMetas` 直前にクラッシュした「インデックス未登録の `.eml`」（孤立ファイル）も次回呼び出し時に自動救済される。ただし `fetch` サブコマンドが渡せるのは `imap.fetch_days` ウィンドウ内のメタ情報のみであるため、それより古い孤立 `.eml` は `fetch` では救済されない。`reprocess`（タスク 0070 F-004 AC-23a）は全 `.eml` を再帰走査するため年齢に関わらず全孤立ファイルを救済できる
...
- `AC-16`: パスは `{root_dir}/emails/{uidvalidity}/{YYYYMM}/{uid}.eml` の形式。`{YYYYMM}` はメールの送信日時（IMAP ENVELOPE の Date フィールド）に基づく
...
- `AC-20`: 指定ディレクトリ以下のすべての `.eml` ファイルを再帰的に列挙できる（ファイルパス `{uidvalidity}/{YYYYMM}/{uid}.eml` から UID・UIDVALIDITY を逆算する）
- `AC-21`: 各エントリは `{Message *mail.Message, UID uint32, UIDValidity uint32, SentAt time.Time, Path string}` 相当の構造体として返す。`SentAt` は `.eml` の `Date:` ヘッダーから取得し、`{YYYYMM}` パスとの整合確認にも使用する（詳細シグネチャは `02_architecture.md` で確定）
