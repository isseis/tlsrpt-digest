...
| **saved_at** | `.eml` ファイルをローカルに保存した日時（wall-clock）。ファイルの inode change time（`ctime`）から取得する。`ctime` は `chmod` などメタデータ変更でも更新されるが、`.eml` は保存後にメタデータ変更を行わない運用前提のため「保存日時」として使用する。Go の標準ライブラリ `os.FileInfo` では ctime が取得できないため、実装上は `syscall.Stat` で OS 固有の stat 構造体から取得する（Linux/Unix のみ対応）。`sent_at` が取得できない場合のフォールバックや GC の最大保持期間判定にも使用する |
...
- `AC-32b`: `savedAtCutoff` が非ゼロの場合、インデックスベースの削除が完了した後に `emails/` 以下の `{uidvalidity}/{YYYYMM}` ディレクトリを走査し、`YYYYMM` が `savedAtCutoff` の年月より前であるディレクトリを `os.RemoveAll` で丸ごと削除する（**孤立 `.eml` の清掃**）。`YYYYMM < savedAtCutoff の年月` の比較は文字列比較で行う（`"202601" < "202603"` 等）。このスイープは `savedAtCutoff` がゼロ値の場合は実行しない。スイープ中の I/O エラーは WARN ログに記録して継続し、`deleted` カウントおよびメインの `err` には反映しない。このスイープにより、Date ヘッダーなしのメールを月をまたいでリトライした際に生じる孤立 `.eml` が最終的に削除される
