...
- ロックファイル（`.tlsrpt-digest-store.lock`）: 機密性は低いが他ファイルと統一
- `.eml` ファイル: メール本文・添付（TLSRPT JSON 本体）

**受け入れ条件**:

- `AC-36`: 本パッケージが作成するすべてのファイルは `0600`（オーナーのみ読み書き可）で作成する。一時ファイル（atomic rename 用）も同様に `0600` とする
- `AC-37`: 本パッケージが作成するすべてのディレクトリ（`{root_dir}` 配下の `emails/` および `{uidvalidity}/`・`{YYYYMM}/` サブディレクトリを含む）は `0700`（オーナーのみアクセス可）で作成する
