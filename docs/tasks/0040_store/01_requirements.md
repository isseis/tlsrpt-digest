...
- `AC-08c`: `SaveEmailMetas` でメールインデックスに `{uid, uidvalidity, received_at, saved_at}` エントリをまとめて登録できる（バッチ操作・1 回のアトミック書き込み）。`received_at` は IMAP ENVELOPE の Date フィールド由来の受信日時（RFC 3339 正規化）を保持し、`{YYYYMM}` パス再構築と GC 時の対象特定に使用する。fetch サイクルで全 `.eml` を保存した後、1 回だけ呼び出すことで O(N²) の JSON 読み書きを回避する
...
`SaveEmail` は `.eml` ファイルの書き込みのみを担う。メールインデックスへの `{uid, uidvalidity, received_at, saved_at}` エントリ登録は、fetch サイクルで全 `.eml` を保存した後に `SaveEmailMetas`（F-002 AC-08c）を 1 回呼ぶことでまとめて行う。これにより SaveEmail ごとの JSON 全読み書きを避け、O(N) の I/O に抑える。`received_at` を保持することで `DeleteEmailsBefore` はインデックスのみで `{uidvalidity}/{YYYYMM}/{uid}.eml` の最終パスを再構築でき、ディレクトリ全走査を不要にできる。TLSRPT パース成功後にバッチ保存メソッド（F-002 AC-08b）が同エントリの `report_end_date` を更新する。
...
累積した `.eml` ファイルを削除する API を提供する。メールインデックス（§6.2）の `report_end_date`・`saved_at`・`received_at` を参照することで、`.eml` 本文やディレクトリ全体をスキャンせずに日単位の精度で削除対象を判定し、削除対象パスを再構築できる。
...
```json
{
  ...
  "emails": [
    { "uid": 123, "uidvalidity": 1234567890, "received_at": "2026-05-12T10:00:00Z", "saved_at": "2026-05-18T10:00:00Z", "report_end_date": "2026-05-12T00:00:00Z" },
    { "uid": 456, "uidvalidity": 1234567890, "received_at": "2026-05-13T11:00:00Z", "saved_at": "2026-05-18T10:01:00Z", "report_end_date": null }
  ]
}
```
...
- `emails`: メールインデックス。fetch サイクル完了後に `SaveEmailMetas` で `{uid, uidvalidity, received_at, saved_at}` エントリをバッチ登録し、バッチ保存メソッド（`SaveReports`）が `report_end_date` を更新する。`received_at` は `{YYYYMM}` パス再構築に使用する。`DeleteEmailsBefore` 時に除去する。`report_end_date` が null のエントリはパース失敗メールを示す。`saved_at` によりパース失敗・遠未来 `report_end_date` に対する最大保持期間での強制削除が可能
