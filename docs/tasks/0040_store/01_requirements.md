...
- `AC-05b`: `SaveReports` はレポートの保存と同時に、各レポートに対応するメールの `{uid, uidvalidity, report_end_date}` をメールインデックスに記録する。`report_end_date` は `tlsrpt.Report.DateRange.EndDatetime` を使用する。インデックス更新もレポート保存と同一のアトミック書き込みに含める。呼び出し元（エントリポイント）から uid・uidvalidity を受け取る形とする（詳細シグネチャは `02_architecture.md` で確定）
...
`SaveEmail` は `.eml` ファイルの書き込みのみを担い、メールインデックスの更新は行わない。インデックス更新は TLSRPT パース成功後に `SaveReports`（F-002）が担う。パースに失敗したメールは `.eml` がディスクに残るが、インデックスには登録されないため `DeleteEmailsBefore`（F-007b）の GC 対象外となる。
...
- `AC-15`: 同一 UID かつ同一 UIDVALIDITY のメールを再度保存しようとした場合、既存ファイルを変更せず、エラーも返さない（冪等動作。正常終了扱い）
...
累積した `.eml` ファイルを削除する API を提供する。JSON データファイル内のメールインデックス（§6.2）に記録された受信日時（`received_at`）を参照することで、`.eml` 本文をスキャンせずに日単位の精度で削除対象を判定できる。

呼び出しタイミングの決定およびスケジューラからの起動は呼び出し元の責務とする。

**受け入れ条件（Acceptance Criteria）**:

- `AC-25`: `DeleteEmailsBefore(cutoff time.Time) (deleted int, err error)` でメールインデックスの `report_end_date < cutoff` に該当する `.eml` ファイルを削除し、削除件数を返す。インデックスに登録されていない `.eml`（パース失敗メール等）は本 API の対象外とし、削除しない
- `AC-26`: 各 `.eml` ファイルを削除した後、対応するインデックスエントリを除去する。インデックスへの書き戻しはアトミック rename で行う
- `AC-27`: `.eml` ファイルが既に存在しない場合もエラーにせず、インデックスエントリのみ除去して処理を継続する（冪等動作）
...
```json
{
  ...
  "emails": [
    { "uid": 123, "uidvalidity": 1234567890, "report_end_date": "2026-05-12T00:00:00Z" }
  ]
}
```
...
- `emails`: メールインデックス。TLSRPT パース成功後の `SaveReports` 時に追記し、`DeleteEmailsBefore` 時に除去する。`report_end_date`（`date-range.end-datetime`）を参照することで `.eml` 本文をスキャンせずに日単位の GC 判定が可能。パース失敗メールはエントリが作成されず GC 対象外（手動確認対象）
