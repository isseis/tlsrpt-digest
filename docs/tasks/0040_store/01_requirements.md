...
- `AC-19`: `SaveUIDValidity(v uint32) error` で UIDVALIDITY を sentinel にアトミックに保存できる
- `AC-20`: `LoadUIDValidity() (v uint32, found bool, err error)` で保存済みの値を取得できる（未保存の場合は `found = false`、`err = nil`）
...
- `uid_validity`: IMAP メールボックスの UIDVALIDITY 値（`uint32`）。IMAP 関連状態は sentinel に集約する。未取得の場合はフィールドを省略し、`LoadUIDValidity` は `found = false` を返す
