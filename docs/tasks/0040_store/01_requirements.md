...
`SaveEmail` はファイル保存と同時に、JSON データファイル内のメールインデックス（§6.2 参照）へ `{uid, uidvalidity, received_at}` エントリを追加する。これにより、`.eml` 本文をスキャンせずに削除対象を日単位で判定できる（F-007b）。JSON インデックスの更新もアトミック rename で行う。
...
- `AC-14a`: メールインデックスへのエントリ追加も `AC-14` と合わせてアトミックに行う（`.eml` rename 成功後に JSON を更新する）
- `AC-15`: 同一 UID かつ同一 UIDVALIDITY のメールを再度保存しようとした場合、`.eml` ファイルもインデックスエントリも変更せず、エラーも返さない（冪等動作。正常終了扱い）
...
- `AC-25`: `DeleteEmailsBefore(cutoff time.Time) (deleted int, err error)` でメールインデックスの `received_at < cutoff` に該当する `.eml` ファイルを削除し、削除件数を返す
...
```json
{
  ...
  "emails": [
    { "uid": 123, "uidvalidity": 1234567890, "received_at": "2026-05-12T10:00:00Z" }
  ]
}
```
...
- `emails`: メールインデックス。`SaveEmail` 時に追記し `DeleteEmailsBefore` 時に除去する。`received_at` を参照することで `.eml` 本文をスキャンせずに日単位の GC 判定が可能
