...
書き込みはアトミックに行う（一時ファイルへ書き込み後に rename）。これにより、書き込み途中でプロセスが停止した場合でも最終パスには完全なファイルのみが存在し、部分ファイルによる不整合が発生しない。
...
- `AC-14`: 書き込みは一時ファイルへ書き込み後に rename するアトミック操作で行う。これにより最終パスには常に完全なファイルのみが存在することを保証する
- `AC-15`: 同一 UID かつ同一 UIDVALIDITY のメールを再度保存しようとした場合、既存ファイルを上書きせず、エラーも返さない（冪等動作。正常終了扱い）
