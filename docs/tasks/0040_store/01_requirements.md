...
| **saved_at** | `.eml` ファイルをローカルに保存した日時（wall-clock）。ファイルのメタデータ（inode change time、`ctime`）から取得する。`sent_at` が取得できない場合のフォールバックや GC の最大保持期間判定にも使用する。実装上、Go の標準ライブラリ `os.FileInfo` では ctime が取得できないため、`syscall.Stat` で OS 固有の stat 構造体にアクセスして ctime を取得する（Linux/Unix のみ対応） |
...
- `AC-21`: 各エントリは `{Message *mail.Message, UID uint32, UIDValidity uint32, SentAt time.Time, SavedAt time.Time, Path string}` 相当の構造体として返す。`SentAt` は `.eml` の `Date:` ヘッダーから取得し、欠損時は `SavedAt` で代用する。`SavedAt` はファイルのメタデータ（inode change time、`ctime`）から取得する。`ctime` はファイルメタデータの最終変更時刻であり、`.eml` は保存後のメタデータ変更（`chmod` 等）を想定しないため、`ctime` で「保存日時」を正確に捉えられる。実装上、`syscall.Stat` で OS 固有の stat 構造体にアクセスして ctime を取得する（reprocess の `SaveEmailMetas` 呼び出しには `{uid, uidvalidity, sent_at, saved_at}` が必要なため）。詳細シグネチャは `02_architecture.md` で確定
