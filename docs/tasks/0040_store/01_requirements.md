...
- `AC-03a`: メール保存ディレクトリ直下の sentinel メタファイル（`.tlsrpt-digest-meta.json`、§6.1 参照）が存在しない場合、現設定の IMAP 識別子（host・port・mailbox）と初期化日時を含めて新規作成する
...
- `AC-12`: ファイル名は `{uid}.eml` 形式とする（Message-ID 等を付与しない）。UID は IMAP `FETCH` で取得した uint32 値を 10 進文字列として使用する
