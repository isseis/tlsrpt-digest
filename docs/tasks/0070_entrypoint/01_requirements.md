...
- **`AC-11d`**: ステップ3の全メール処理が正常に完了した後、ステップ1で取得した現在の `UIDVALIDITY` を `internal/store` の `SaveUIDValidity(mailbox, v)` で保存する（次回実行時の比較に使用）。正常完了前に保存しないことで、クラッシュ後の再実行時に強制ダウンロード（`AC-11c`）が確実に発動する
