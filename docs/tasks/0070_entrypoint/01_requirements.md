...
- `AC-32b`: `internal/store` の `DeleteEmailsBefore(time.Now().Add(-maxEmailAge))` を呼び出して `internal_date` ベースで `.eml` ファイルおよびメールインデックスを削除する（タスク 0041 F-001 で単一カットオフ化済み）
...
- `gc` 処理フローのテスト（`FakeStore`・スパイハンドラ を使用）
  ...
  - `--max-email-age` 指定時に `DeleteEmailsBefore` が正しいカットオフ時刻で呼ばれること（タスク 0041 で単一カットオフ化済み）
