...
- `AC-07d`: `summary` サブコマンドの集計窓の**終端**は、現在時刻を UTC 日付の開始時刻（今日の 00:00:00 UTC）に切り捨てた日時とする。これにより開始（AC-07c）・終端の両方が UTC 暦日境界に揃い、週次実行での重複・欠落が生じない。例：`--since 1w` を 2000-12-10 10:00 UTC に実行した場合の窓は `[2000-12-03 00:00 UTC, 2000-12-10 00:00 UTC)`
