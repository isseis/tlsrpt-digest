...
- `AC-10c`: `summary` はストアのロックを取得しない。ただしこれが成立するのは、`summary` がストアを **ファイル作成なし（read-only open）モード** で開く場合に限る。read-only open ではファイルが存在しない場合に新規作成を行わず、エラーも返さない（ストア自体が未作成の場合は「集計対象データなし」として正常終了する）。read-only open と通常の初期化（F-001、ファイル作成あり）の両方を提供することは `internal/store` の責務とする（0040 F-001 参照）。`summary` がファイルを作成しないことにより、`fetch`・`gc` との書き込み競合が発生しないことが保証される

### F-003: `fetch` サブコマンドの処理フロー

IMAP サーバーからメールを取得し、レポートを処理・保存する。常に設定期間（`imap.fetch_days`、`--since` で上書き可）内の全メールを対象とし、SEEN フラグとローカル `.eml` ファイルの有無のみでダウンロード要否を判定する。ただし `imap.MaxMessageBytes`（0010 `Config.MaxMessageBytes`）が設定されている場合、RFC822.SIZE がその上限を超えるメールは DoS 対策として IMAP ライブラリ層でダウンロード対象から除外される（WARN ログあり）。これとは別に、**ローカルファイルとの RFC822.SIZE 不一致はダウンロード判定に影響しない**（AC-13 参照）。
...
| クラッシュタイミング | 次回実行の結果 |
|---|---|
| AC-11b `SaveUIDValidity` 完了後・ステップ3前 | sentinel に `UIDVALIDITY` あり → 次回は正常扱い（`UIDVALIDITY` 変化があれば検出可能） |
