...
7. `--since` の duration は日単位（`d`）または週単位（`w`）で指定できる（例：`30d`、`4w`）
...
|---|---|---|---|
| UNSEEN | なし | — | ダウンロード対象 |
| UNSEEN | あり | 一致 | ダウンロード対象（処理済みだが SEEN マークが未完） |
...
5. ダウンロード対象のメールをまとめて取得し、メール原本を `.eml` ファイルに保存する（既存ファイルは上書きしない）
...
- `fetch` 処理フローのテスト（`FakeMailFetcher`・`SpyNotifier`・`FakeStore` を使用）
  ...
  - UNSEEN → ダウンロード・処理・SEEN マークが行われること
