...
- `AC-07c`: すべての duration フラグによるカットオフ日時は、現在時刻を **UTC 日付の開始時刻（00:00:00 UTC）に切り捨ててから**指定日数を遡った日時とする。これにより、外部スケジューラの起動タイミングのジッターが集計窓・保持期間の境界に影響しない。例：`--since 7d` を 02:01:00 に実行した場合のカットオフは「7 日前の 02:01:00」ではなく「7 日前の 00:00:00 UTC」となる
...
- `AC-27`: `--since <duration>` または設定値で指定された期間のレポートをストアから取得して集計する。カットオフ日時の計算は `AC-07c` に従い UTC 日付単位で切り捨てる
...
- duration カットオフ計算テスト（AC-07c）
  - 現在時刻が UTC 02:01:00 のとき `7d` のカットオフが「7 日前の 00:00:00 UTC」となること（「7 日前の 02:01:00」ではないこと）
  - 週指定（例：`1w`）でも同様に UTC 日付単位の切り捨てが行われること
