...
// store.Store は discard-old 復旧と abort-reset の公開仕様として
// ResetForRecovery と AbortReset を直接持つ。recover は type assertion せず Store 経由で呼ぶ。
// 両 API ともに OpenRecoverReset で開かれた store でのみ呼び出し可能。
type Store interface {
    ResetForRecovery(currUIDValidity uint32) error
    AbortReset(currUIDValidity uint32) error
}
...
| `cmd/tlsrpt-digest/recover.go` | F-007: UIDVALIDITY 変化からの手動復旧。実行前にオペレータ向け情報（recovery-required 内容・復旧モード・データパス・pending reset 状態）を stdout に表示する。`keep-old` は `ApplyRecovery`、`discard-old --yes` は `internal/store` の破棄復旧 API を呼ぶ。`--abort-reset --yes` は pending reset を commit 前にロールバックする escape hatch を提供する（`internal/store.Store.AbortReset(currUIDValidity)` を呼ぶ）。 | 新規追加 |
...
| `internal/store/store.go` | `store.Store` に `ResetForRecovery(currUIDValidity uint32) error` と `AbortReset(currUIDValidity uint32) error` を追加し、破棄復旧の永続化境界を store パッケージ内へ閉じ込める。`Open` 関数で `OpenRecoverReset` mode を認識・処理する分岐を追加する。 | 変更あり |
| `internal/store/recovery.go` | `ResetForRecovery` の 5 フェーズ state machine と pending reset の fail closed / recover-only 再開境界、および `AbortReset` の対称的ロールバック（②③ を逆方向に実行して staging から原位置へ戻す）を実装する。 | 変更あり |
...
| `recover` | `--yes` | `discard-old` または `--abort-reset` の実行確認 | - |
| `recover` | `--abort-reset` | pending reset を commit 前にロールバックする escape hatch（`--yes` 必須、`--mode` と排他） | - |
...
flowchart TD
    ...
    ABORT["abort-reset --yes<br>commit 前のみ"]
    ABORT_END(["exit 0<br>recovery-required 残存<br>keep-old / discard-old 再選択可"])

    START --> P1 --> P2 --> P3 --> COMMIT --> DONE --> END
    P1 & P2 & P3 -.->|"クラッシュ"| CRASH_PRE
    COMMIT & DONE -.->|"クラッシュ"| CRASH_POST
    CRASH_PRE -->|"discard-old --yes 再実行"| P1
    CRASH_PRE -->|"abort-reset --yes"| ABORT
    P1 & P2 & P3 -->|"abort-reset --yes"| ABORT
    ABORT --> ABORT_END

    class P1,P2,P3 phase
    class COMMIT commitnode
    class DONE,END,ABORT_END done
    class CRASH_PRE,CRASH_POST problem
    class ABORT commitnode
```

矢印 A → B は正常実行時のフェーズ遷移、破線矢印はクラッシュ発生時の到達状態、`abort-reset --yes` ラベルの矢印は手動ロールバックを表す。`abort-reset --yes` は commit 前（①②③または `CRASH_PRE`）からのみ実行可能で、commit 後（④⑤）からは拒否される。
...
|---|---|---|---|
| ①②③ でクラッシュ（commit 前） | 旧データが残存（または未作成） | (a) `discard-old --yes` 再実行で完走 / (b) `abort-reset --yes` でロールバック | (a) の場合は ⑤ で削除、(b) の場合は abort 処理内で削除（次項参照） |
| ④⑤ でクラッシュ（commit 後） | 空ストア有効、staging が残存 | 次回 `Open`（任意モード） | `Open` 時の後片付けで削除 |
| commit 前に操作を中断し `keep-old` へ切り替えたい | 旧データが staging に存在 | `abort-reset --yes` 実行後に `keep-old` を実行 | abort により旧位置へ復元・staging 削除、その後 `keep-old` は通常通り動作 |

**`abort-reset --yes` の動作**（AC-42 / AC-43 / AC-44 / AC-45）

- **受理条件**: pending reset が存在し commit 前（manifest あり・sentinel 未更新）の状態のみ。それ以外は説明付きで exit 1。
- **動作**: ② で staging へ退避済みの `tlsrpt.json` と `emails/` を逆方向に rename して元の位置に戻す。空作成済みのファイル・ディレクトリ（③）は abort 前に削除する。最後に manifest を削除する。完了後は recovery-required 状態が残り、オペレータは `keep-old` または `discard-old --yes` を再選択できる。
- **冪等再実行**: abort 自体も中断され得るため、簡易な abort 用 manifest（例: `abort_in_progress`）を残してべき等に再実行できるよう設計する。具体的な abort phase の境界・名前は実装計画で確定する。
- **commit 後の拒否**: ④ 以降では abort してはならない。sentinel が既に新 UIDVALIDITY を保持しており、staging を逆移動すると整合性が失われるため。

この escape hatch により、commit 前であればオペレータが `discard-old --yes` を放棄して `keep-old` へ切り替えることが可能となる。staging データの「移動」操作は対称的に逆実行できるため、`abort-reset` の実装は新たな破壊的境界を追加しない。
...
| reset incomplete | pending reset があり通常 open が拒否された | `SystemErrorKind=reset_incomplete` を通知し、`recover --mode discard-old --yes` の継続または `recover --abort-reset --yes` のロールバックを案内 |
| reset failure | store 内部の reset 再開または commit に失敗 | `recovery_required` を残し、再実行可能な状態で exit 1 |
| abort after commit | `AbortReset` が commit 後の状態に対して呼ばれた | 「commit 後は abort できない」と説明し exit 1（staging 残存時は次回 Open で cleanup される旨も案内） |
| abort failure | `AbortReset` の対称ロールバック中に失敗 | abort manifest を残して再実行可能な状態で exit 1（再実行で復旧） |
...
| pending reset 状態 | manifest 存在時はフェーズ名（`prepared` / `moved` / `empty_created`）と「commit 前/後」の区分を表示。なければ「なし」 |

上記の共通表示に加え、モードごとに以下を追加表示する。pending reset が検出された場合は、利用可能な選択肢として `discard-old --yes` の継続実行と `abort-reset --yes` のロールバックを併せて案内する（AC-45）。
...
- **`abort-reset --yes`**（commit 前 pending reset のみ）: staging から元の位置へ戻すこと、manifest を削除すること、abort 完了後は recovery-required 状態が残るため `keep-old` または `discard-old --yes` を再選択できることを表示してから abort を実行する。完了時に「`abort-reset` 完了。次回 `recover --mode keep-old` または `recover --mode discard-old --yes` を選択してください」と stdout に表示する。
...
| `recover` | `store.Open(OpenReadWrite)`（通常確認・`keep-old`） / `store.Open(OpenRecoverReset)`（pending reset 再開を許可する `discard-old --yes` / `abort-reset --yes` 専用） / `LoadRecoveryRequired` / `ApplyRecovery`（`keep-old`） / `ResetForRecovery(currUIDValidity)`（`discard-old --yes`） / `AbortReset(currUIDValidity)`（`abort-reset --yes`） |
...
- **`recover.go`** — AC-35–45
  ...
  - `--abort-reset --yes` で pending reset を commit 前ロールバックし、その後 `keep-old` が実行できること（AC-42 / AC-43）
  - `--abort-reset` 単独・`--yes` 単独はエラー扱いになること（AC-42）
  - commit 後の状態に対する `--abort-reset --yes` は拒否されること（AC-43）
  - pending reset 検出時に状況と選択肢（継続 / abort）が stdout に表示されること（AC-45）
- **`internal/store` の復旧 API** — AC-38 / AC-41 / AC-43 / AC-44
  ...
  - commit 前の中間失敗時に recovery-required 状態と staging 退避済みデータが残り、次回 `recover --mode discard-old --yes` または `recover --abort-reset --yes` を再実行できること
  - commit 後の staging 削除失敗が通常データパスへ影響しないこと
  - `AbortReset(currUIDValidity)` が ②③ の対称的ロールバック（staging から原位置への戻し、空作成済みファイル・ディレクトリの削除、manifest 削除）を store パッケージ内で行うこと
  - abort 自体の中間失敗時にも、次回 `AbortReset` 再実行で同じ最終状態（recovery-required 残存・staging なし・空作成済みなし）へ収束すること（AC-44 冪等性）
  - commit 後（④以降）の `AbortReset` 呼び出しが拒否されること
