...
| NotificationSink | `cmd` レイヤーが通知に使える facade。Slack handler を直接公開せず、型付き helper だけを呼べるようにする |
| typed helper | `internal/notify` が提供する `LogAlert` / `LogWarning` / `LogSystemError` / `LogSummary`。Slack payload を型で制限する境界 |
| SlackHandler | `internal/notify` 内部の送信・バッファリング実装。`cmd` レイヤーやサブコマンドへ直接渡さない |
...
矢印 A → B は同一 `package main` 内の呼び出し・責務依存を表す。Go の file 間 import ではない。`duration.go` は `fetch` / `summary` / `gc` の 3 サブコマンドから共有される。
...
sequenceDiagram
    ...
    alt 書き込み系サブコマンド (fetch/gc/recover/reprocess)
        ...
        Note over B,S: summary consistency guard 取得 + 第 1 回 CheckRecoveryRequired（[6.7] ステップ 2）
        B->>S: AcquireSummaryConsistencyGuard()
        B->>S: CheckRecoveryRequired()
        S-->>B: found / not found
        ...
        alt Summary が空（[6.7] ステップ 4a）
            ...
            N-->>B: []*SlackHandler or error (all-or-nothing)
            B->>B: NotificationSink facade でラップ
            Note over B,S: 第 2 回 CheckRecoveryRequired・Slack 送信直前（summary.go 実行、[6.7] ステップ 5）
            B->>S: CheckRecoveryRequired()
            S-->>B: found / not found
...
// RecoveryResetStore は既存 store.Store に追加される discard-old 復旧能力。
// recover は store.Store がこの能力を持つ前提で、type assertion せず Store 経由で呼ぶ。
type RecoveryResetStore interface {
    ResetForRecovery(currUIDValidity uint32) error
    AbortReset() error
}

// SummaryConsistencyGuard は summary がロックを取得せずに read-only 集計と送信可否を結びつける境界。
// Guard が有効な間、store は recovery-required の出現を送信直前に検出可能な形で保証する。
...
| `cmd/tlsrpt-digest/lock.go` | store 単位のプロセス排他ロックの取得・保持・テスト用 `Close`。具体的な OS API 選定は実装計画で扱う。 | 新規追加 |
...
| `cmd/tlsrpt-digest/summary.go` | F-005: 期間を解決し（`start = Duration.Cutoff(now)`、`end = UTCDayStart(now)`）、read-only consistency guard で recovery-required を確認し、既存 `notify.GenerateSummary(ctx, store, start, end, debugLogger)` を呼んで `NotificationSink.LogSummary` 経由で Slack へ送信する。集計ロジックは再実装しない。 | 新規追加 |
| `cmd/tlsrpt-digest/reprocess.go` | F-004: 保存済み `.eml` の再パースとストア再構築。`--notify` 指定時は TLS failure を `NotificationSink.LogAlert`、ファイル単位の読み込み・パース失敗を `NotificationSink.LogWarning` で通知する。 | 新規追加 |
...
| `cmd/tlsrpt-digest/recover.go` | F-007: UIDVALIDITY 変化からの手動復旧。実行前にオペレータ向け情報（recovery-required 内容・復旧モード・データパス・pending reset 状態）を stdout に表示する。`keep-old` は `ApplyRecovery`、`discard-old --yes` は `ResetForRecovery`、`--abort-reset --yes` は `AbortReset` を呼ぶ。 | 新規追加 |
...
| `cmd/tlsrpt-digest/recover_test.go` | `keep-old` / `discard-old --yes` / `--abort-reset --yes` / `--yes` 不足 / recovery-required 不在 / pending reset 表示の単体テスト。 | 新規追加 |
...
| `internal/store/types.go` | `OpenRecoverReset` mode と read-only consistency guard の公開 contract を定義する。 | 変更あり |
| `internal/store/store.go` | `store.Store` に `ResetForRecovery(currUIDValidity uint32) error`、`AbortReset() error`、summary guard 取得 API を追加し、破棄復旧と abort の永続化境界を store パッケージ内へ閉じ込める。`Open` 関数で `OpenRecoverReset` mode を認識・処理する分岐を追加する。 | 変更あり |
| `internal/store/recovery.go` | `ResetForRecovery`、`AbortReset`、pending reset の fail closed / recover-only 再開境界を実装する。 | 変更あり |
| `internal/store/store_test.go` | `OpenRecoverReset` と通常 `OpenReadWrite` の pending reset 扱いの違いを検証する。 | 変更あり |
| `internal/store/recovery_test.go` | `ResetForRecovery` / `AbortReset` の更新範囲、pending reset の open 境界、cleanup、中間失敗時の再実行性を検証する。 | 変更あり |
| `internal/store/testutil/mocks.go` | `store.Store` に追加される `ResetForRecovery`、`AbortReset`、summary guard を fake/mock に実装する。 | 変更あり |
...
**プロセスロックの契約**
- **ロック識別子**: store ごとに一意なロック対象を使い、ストアディレクトリ外の共有資源には依存しない。具体的なファイル名や作成方法は実装計画で扱う。
- **sentinel と分離する理由**: sentinel ファイル（`.tlsrpt-digest-meta.json`）は store の整合性管理対象であり、復旧処理で更新される。専用ファイルへ分離することでロックの責務と store データの責務を独立させる。

**取得・解放の契約**
- **取得方式**: 書き込み系サブコマンドは store 単位の排他ロックを取得する。取得できない場合は他プロセス保持中とみなし、待機せず `LogSystemError` + `Flush()` + exit 1 とする。複数プロセスが同時にロック取得を試みた場合、それぞれが独立して `lock_held` 通知を Slack へ送信し得る（重複は許容する）。具体的な OS API は実装計画で確定する。
...
書き込み系サブコマンドの**実際の実行順序**は `W-1 → W-2 → W-3 → W-4 → W-5 → W-6`、`fetch` のみさらに `W-6f`（IMAP 認証情報取得）が続く。以下のテーブルの行順は実行順と一致する。
...
| W-6f | `fetch` のみ、recovery-required 確認後かつ IMAP 接続直前に `TLSRPT_IMAP_USERNAME` を `string` として、`TLSRPT_IMAP_PASSWORD` を即座に `config.Secret` でラップして取得する（ADR-0002 §4.3）。`gc` / `recover` / `reprocess` / 空ストア `summary` は IMAP 認証情報を要求しない | `LogSystemError` + `Flush()` + exit 1 |

**`summary` の初期化フロー**（詳細は [§6.7](#67-summary-の空ストア時シーケンス)）

W-2（ディレクトリ作成）・W-5（ロック取得）はスキップし、以下の順で実行する。

1. W-1（設定読込）→ `store.Open(OpenReadOnly)` → summary consistency guard 取得 → `CheckRecoveryRequired`（第 1 回確認）
...
3. **recovery-required なし** → `notify.GenerateSummary(start=Duration.Cutoff(now), end=UTCDayStart(now))` を呼ぶ
   - **集計結果が空** → guard の `CheckRecoveryRequired` を再確認（第 2 回・空パス用）
     - 見つかった場合: stderr ERROR + exit 1（notifier 未構築のため Slack 通知なし）
     - 見つからない場合: INFO ログ「集計対象なし」→ exit 0
   - **集計結果が非空** → W-3・W-4 で notifier 構築 → guard の `CheckRecoveryRequired` を再確認（第 2 回・非空パス用）
...
| `recover` | `--yes` | `discard-old` / `abort-reset` 実行確認 | - |
| `recover` | `--abort-reset` | pending reset の取り消し。`--yes` と組み合わせた場合のみ有効 | - |
...
**`gc` の `--before` と `--max-email-age` を独立に持つ理由**: JSON レポートレコードと `.eml` 原本では削減ポリシーの粒度が異なる。レポートレコードは集計に必須なので長めに残し、`.eml` 原本は再パース可能性とストレージ消費の運用トレードオフとして別に調整できるようにする。
...
graceful shutdown は要件定義書の対象外であるため、本タスクでは SIGTERM / SIGINT 専用のハンドリングを追加しない。プロセスが外部から終了された場合、実行中コマンドのロックは解放される。At-least-once 保証は [4.3](#43-flush-失敗時の取扱い) と [6.1](#61-fetch-の-at-least-once-保証) の通常処理順序、すなわち `Flush()` 成功確認後に SEEN を付与する順序で担保する。
...
プロセス全体のタイムアウトは本タスクでは実装しない。One-shot 実行の停止期限は外部スケジューラ（例: systemd service の `TimeoutStartSec` / `RuntimeMaxSec`、cron 側の監視）で管理する。アプリケーション内に独自の全体 timeout と Flush grace period を持ち込まない。
...
本タスクは Slack transport を `internal/notify` に委譲するが、`cmd` レイヤーは通知イベントを生成する。そのため以下の点で [Notification Security Guidelines](../../dev/developer_guide/notification_security.md) の遵守が必要となる。
...
- **Principle 1（型制約）**: `boot.go` は `notify.BuildHandlers` の戻り値を即座に `NotificationSink` facade へラップし、サブコマンドへ `slog.Handler` / `*notify.SlackHandler` として渡さない。facade のメソッドは `LogAlert` / `LogWarning` / `LogSystemError` / `LogSummary` / `Flush` のみであり、内部実装が `internal/notify` の型付きヘルパーを呼ぶ。エントリポイント側で `logger.Info(...)` などの非型付き API を Slack ハンドラ向けに呼ぶ箇所は型上作れない。
  ...
  - **SystemError payload**: `LogSystemError` は `SystemErrorKind`・component・mailbox 識別子のみを payload に含める。raw `err.Error()` や任意の message 文字列は含めない。詳細診断は stderr/debug logger と README の固定文言に分離する。UIDVALIDITY 不一致は `uidvalidity_changed`、recovery-required 残存は `recovery_required`、pending reset は `reset_incomplete` を使う。
...
ロック対象はストアディレクトリの権限境界内に置き、外部ユーザが任意に差し替えられる共有場所へ置かない。具体的な作成モードやファイル属性は実装計画で扱う。
...
### 6.4 `recover` の破壊的復旧境界と不変条件

`discard-old --yes` と `--abort-reset --yes` は複数ファイルにまたがる破壊的操作であるため、永続化境界は `internal/store` に閉じ込める。`cmd/tlsrpt-digest/recover.go` は `ResetForRecovery(currUIDValidity)` または `AbortReset()` を呼ぶだけにし、store 内部のパスやファイル形式を直接扱わない。
...
`recover --mode keep-old` は既存 `ApplyRecovery(curr)` を使い、`discard-old --yes` はデータ破棄を伴うため `ResetForRecovery(currUIDValidity)` を使う。`--abort-reset --yes` は pending reset を取り消すため `AbortReset()` を使い、完了後も recovery-required は残す。
...
| reset incomplete | pending reset があり通常 open が拒否された | `SystemErrorKind=reset_incomplete` を通知し、`recover --mode discard-old --yes` の継続を案内 |
| reset failure | store 内部の reset 再開または commit に失敗 | `recovery_required` を残し、再実行可能な状態で exit 1 |
| abort unavailable | pending reset がない、または commit 後で abort できない | オペレータへ「abort 不可」と表示し exit 1 |
| abort failure | store 内部の abort に失敗 | `recovery_required` を残し、再実行可能な状態で exit 1 |

更新範囲は以下:

| 対象 | 操作 |
|---|---|
| レポート store | `discard-old --yes` では空のレコードセットへ置き換える。`--abort-reset --yes` では commit 前の pending reset を取り消し旧データを保持する |
| `.eml` store | `discard-old --yes` では空の状態へ置き換える。`--abort-reset --yes` では commit 前の pending reset を取り消し旧データを保持する |
| sentinel `imap_host` / `imap_port` / `imap_mailbox` | 変更しない（メールボックス紐付けの履歴を保持） |
| sentinel `initialized_at` | 変更しない（運用開始日時の履歴を保持） |
| sentinel `uid_validity` | `discard-old --yes` では recovery-required 状態に記録された現在値へ更新。`--abort-reset --yes` では変更しない |
| sentinel `recovery_required` | `discard-old --yes` ではフィールドを除去。`--abort-reset --yes` では残す |

**中間クラッシュからの再実行性**:

commit 前にクラッシュした場合は recovery-required を残し、通常コマンドは fail closed する。`recover --mode discard-old --yes` の再実行は reset を継続し、`recover --abort-reset --yes` は commit 前の reset を取り消す。commit 後に cleanup が残った場合は、通常データパスは空ストアとして一貫しており、後続の `Open` または reset 実行時に後片付けできる。
...
| selected mode | `keep-old` / `discard-old` / `abort-reset` |
| pending reset 状態 | 未完了 reset があれば「あり」、なければ「なし」 |

上記の共通表示に加え、モードごとに以下を追加表示する。pending reset が検出された場合は、利用可能な選択肢として `discard-old --yes` の継続実行と `abort-reset --yes` のロールバックを案内する。

- **`keep-old`**: 旧 UIDVALIDITY エポック由来の既存レポートと `.eml` は保持され、期間条件に一致する限り今後の `summary` に混入し得る旨を警告する。
- **`discard-old`（`--yes` なし）**: レポート store と `.eml` store が空状態へ置き換わること、sentinel の `uid_validity` が current へ更新されることを表示する。破壊的変更は行わず exit 1 とする（ドライラン表示）。
- **`discard-old --yes`**: 上記と同じ内容を表示してから `ResetForRecovery(currUIDValidity)` を呼ぶ。
- **`--abort-reset --yes`**: pending reset を取り消し、recovery-required を残したまま `keep-old` または `discard-old --yes` を再選択できる状態へ戻す。`--abort-reset` 単独または `--yes` 単独は exit 1 とする。
...
| `summary` | `store.Open(OpenReadOnly)` / summary consistency guard 取得 / `CheckRecoveryRequired` / `notify.GenerateSummary` / `NotificationSink.LogSummary` / `NotificationSink.LogSystemError` / `NotificationSink.Flush` |
| `reprocess` | `store.Open(OpenReadWrite)` / `LoadRecoveryRequired` / `LoadEmails` / `SaveEmailMetas` / `SaveReports` /（`--notify` 指定時のみ）`NotificationSink.LogAlert` / `NotificationSink.LogWarning` / `NotificationSink.Flush` |
...
| `recover` | `store.Open(OpenReadWrite)`（通常確認・`keep-old`） / `store.Open(OpenRecoverReset)`（pending reset を扱う `discard-old --yes` / `--abort-reset --yes` 専用） / `LoadRecoveryRequired` / `ApplyRecovery`（`keep-old`） / `ResetForRecovery(currUIDValidity)`（`discard-old --yes`） / `AbortReset()`（`--abort-reset --yes`） |
...
2. read-only consistency guard を取得し、`CheckRecoveryRequired` を必ず先に呼ぶ（第 1 回確認）。`found = true` の場合は空ストア扱いにせず、§3.4 の W-3・W-4 を実行して notifier を構築してから `SystemErrorKind=recovery_required` を送信し exit 1 とする（§2.2 参照）。
...
4a. 生成された `Summary` が空の場合、guard の `CheckRecoveryRequired` を再確認する（第 2 回確認・空パス用）。`found = true` ならば stderr に ERROR ログを出力して exit 1（notifier 未構築のため Slack 通知なし、AC-27a 準拠）。`found = false` ならば Slack notifier を構築せず INFO レベル `slog` ログ「no reports to summarize」を 1 行出力し exit 0。
4b. `Summary` が非空の場合、§3.4 の W-3・W-4 に従い Slack URL 取得と `BuildHandlers` を実行して notifier を構築する。
5. Slack 送信直前に guard の `CheckRecoveryRequired` を再実行する（第 2 回確認・非空パス用）。recovery-required が出現していれば `SystemErrorKind=recovery_required` を通知して exit 1 とする。`found = false` の場合だけ `LogSummary` / `Flush` へ進む。guard はこの判定と送信開始の間で writer が recovery-required を作成した場合に false negative を返さない contract を持つため、`summary` がプロセス排他ロックを取得しない設計でも fail closed 境界を持つ。
6. exit 0。

ストアは存在するがレポート 0 件の場合（`notify.GenerateSummary` が空 Summary を返し、guard の `CheckRecoveryRequired` も `found = false`）も同じ挙動とする。
...
- **`summary.go`** — AC-07a / AC-07c / AC-07d / AC-10c / AC-27 / AC-27a / AC-28 / AC-29
  ...
  - 集計前と Slack 送信直前に summary consistency guard で recovery-required を確認し、並走する `fetch` が recovery-required を作った場合は送信せず exit 1 になること
  - `notify.GenerateSummary` を呼び、`summary.go` では集計ロジックを再実装しないこと
- **`reprocess.go`** — AC-21a / AC-22–26
  - recovery-required ガード
  - `--notify` 指定時の TLS failure は `LogAlert`、ファイル単位読み込み・パース失敗は `LogWarning` で通知されること
...
- **`recover.go`** — AC-35–45
  ...
  - previous/current UIDVALIDITY、メールボックス、local data path、選択 mode、pending reset 状態が stdout に表示されること
  ...
  - `--abort-reset --yes` で `internal/store.Store.AbortReset()` が呼ばれること
  - `--abort-reset` 単独・`--yes` 単独が破壊的変更を行わず exit 1 になること
  - pending reset 検出時に `discard-old --yes` と `abort-reset --yes` の選択肢が stdout に表示されること
  - recovery-required 不在時の説明付き exit 1
- **`internal/store` の復旧 API** — AC-38 / AC-41 / AC-43 / AC-44
  - `ResetForRecovery(currUIDValidity)` が「旧データ保持 + recovery-required 残存」または「空ストア + current UIDVALIDITY + recovery-required 解消」のどちらかに収束すること
  - `AbortReset()` が commit 前の pending reset だけを取り消し、「旧データ保持 + recovery-required 残存」に収束すること
  - pending reset がない場合または commit 後は `AbortReset()` が変更せずエラーを返すこと
  - 再実行しても最終状態へ収束すること
  - pending reset では通常 `OpenReadWrite` が fail closed し、`OpenRecoverReset` だけが再開用 store を返すこと
  - commit 後の cleanup 未完了が通常データパスへ影響しないこと
