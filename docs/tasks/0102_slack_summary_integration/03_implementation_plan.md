...
| `slackNotifyWebhookEnvKey` | `cmd/tlsrpt-digest/slack_notify_env_test.go:13` | `"TLSRPT_SLACK_WEBHOOK_URL_ERROR"` を指す既存定数。`missingSlackSummaryEnv` でも再利用する（後述）。 |
| `missingSlackNotifyEnv` / `TestSlackNotify_EnvRequirements` | `cmd/tlsrpt-digest/slack_notify_env_test.go` | 追加する `missingSlackSummaryEnv` の設計パターン。変更不要（追記のみ）。 |
...
| ファイル | 種別 | 内容 |
|---|---|---|
| `cmd/tlsrpt-digest/slack_notify_env_test.go` | 追記 | `slackSummaryWebhookEnvKey` 定数と `missingSlackSummaryEnv` 関数、`TestSlackSummary_EnvRequirements` テストを追加 |
| `cmd/tlsrpt-digest/slack_summary_integration_test.go` | 新規作成 | `loadSlackSummaryTestEnv`・`TestSlackSummary_Summary_Integration` |
| `Makefile` | 追記 | `test-slack-summary` ターゲットと `.PHONY` 追加 |

#### 確認済みシンボル・設計判断

- `slackNotifyWebhookEnvKey = "TLSRPT_SLACK_WEBHOOK_URL_ERROR"` が `slack_notify_env_test.go` 行 13 に定義済み。`missingSlackSummaryEnv` はこの既存定数を再利用して error URL を確認する。同パッケージ内で同一文字列値を持つ別定数を追加すると重複・混乱の原因となるため、新たな error 用定数は追加しない。
- `notify.ValidateEnvCombination` は `successURL != "" && errorURL == ""` の組み合わせを拒否する（`internal/notify/validate.go:15`）。そのため `missingSlackSummaryEnv` は **両方の URL**（`slackSummaryWebhookEnvKey` と `slackNotifyWebhookEnvKey`）の設定を必須とし、`loadSlackSummaryTestEnv` はどちらかが欠けている場合もスキップする。
...
- アーキテクチャ設計書 `02_architecture.md` には以下の 3 箇所に誤りがあり、本計画書の記述が正しい:
  ...
  - §3.2 コードスニペット: `slackSummaryErrorWebhookEnvKey` を新規定数として宣言しているが、既存の `slackNotifyWebhookEnvKey` を再利用するため追加不要（本計画書タスク 1.1〜1.2 参照）。
  - §6.2 フローチャートノード G: `"setupNotifyHandlers successURL のみ設定"` → 正しくは `successURL + errorURL` の両方を渡す。
  上記の誤りは `02_architecture.md` 自体でも訂正済みである（定数スニペットにコメント追記、フローチャートラベル修正）。
...
- [ ] **1.1** `slackSummaryWebhookEnvKey` 定数を追加する。
  - 値: `"TLSRPT_SLACK_WEBHOOK_URL_SUCCESS"`
  - 既存の `slackNotifyWebhookEnvKey` 定数の直後に追加する。

- [ ] **1.2** `missingSlackSummaryEnv(env map[string]string) []string` 関数を実装する。
  - `slackSummaryWebhookEnvKey`（`"TLSRPT_SLACK_WEBHOOK_URL_SUCCESS"`）と `slackNotifyWebhookEnvKey`（`"TLSRPT_SLACK_WEBHOOK_URL_ERROR"`）の**両方**を確認する。
...
- [ ] **2.3** `loadSlackSummaryTestEnv(t *testing.T) (successURL, errorURL string)` 関数を実装する。
  ...
  - `os.Getenv(slackSummaryWebhookEnvKey)` と `os.Getenv(slackNotifyWebhookEnvKey)` の両方を返す。
...
フェーズ 1 はフェーズ 2 の前提（`missingSlackSummaryEnv` を `loadSlackSummaryTestEnv` が利用する）。
...
本タスクは既存ファイルに `slack_notify_env_test.go` への **追記**（既存行の変更なし）と Makefile への **追記** のみを行う。`slack_notify_integration_test.go` は変更しない。
...
| リスク | 影響 | 対策 |
|---|---|---|
| `ValidateEnvCombination` が success URL のみの設定を拒否する | `setupNotifyHandlers` が失敗しテストがエラー終了 | `missingSlackSummaryEnv` で error URL（`slackNotifyWebhookEnvKey`）の設定も必須にし、両方の URL を `loadSlackSummaryTestEnv` で取得・渡す |
| `successURL` と `errorURL` が異なるホスト名を持つ | `BuildHandlers` が `errorURL` の AllowedHost 検証で失敗 | `cfg.Notify.Slack.AllowedHost` は `successURL` のホスト名から設定するため、両 URL が同一ホストである前提となる。異なるホストを使用すると `setupNotifyHandlers` がエラーを返す。手動検証時は同一 Slack ワークスペースの URL を使用することを README / コメントで明記すること |
...
- [ ] `slackSummaryWebhookEnvKey` 定数追加（`slack_notify_env_test.go`）
- [ ] `missingSlackSummaryEnv` 関数実装（`slackSummaryWebhookEnvKey` と `slackNotifyWebhookEnvKey` の両 URL チェック）
...
- [ ] `loadSlackSummaryTestEnv` 関数実装（`slack_summary_integration_test.go` にのみ配置、`slack_notify_env_test.go` には追加しない）
...
1. フェーズ 1 実装（`/runplan` コマンドで実施）。
2. フェーズ 2 実装（`/runplan` コマンドで実施）。
3. 手動検証: `TLSRPT_SLACK_WEBHOOK_URL_SUCCESS` と `TLSRPT_SLACK_WEBHOOK_URL_ERROR` を設定して `make test-slack-summary` を実行し、Slack チャンネルでサマリメッセージの書式を目視確認する。
