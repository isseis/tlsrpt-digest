...
- **YAGNI / DRY**: フェーズ 5 を残すための特殊扱いをすべて削除する。具体的には `ResetForRecovery` のフェーズ 5 拒否チェック・`restoreFromStaging` 関数・`AbortReset` 専用のエラー型・CLI 経路を削除し、判定を単純化する。なお `HasPendingReset` の比較式 `mfst.Phase != resetPhaseCommitted` 自体は削除しない。有効フェーズが `{1, 4}` に縮小されることで、この式は実質「フェーズ 1 のみが保留」という単純な意味になるため、コメントと意味づけを整合させる（コード削除ではなく意味のクリーンアップ）。
