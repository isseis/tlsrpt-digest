...
1. 通知イベントは型付きヘルパー関数（例：`LogAlert(ctx context.Context, alert Alert) error`）を通じてのみ内部ロガーへ書き込む。外部コードは直接 `logger.Info(...)` を呼ばず、必ずこれらのヘルパーを使う
...
`slog.Handler.Handle()` が返す `error` は slog の呼び出し元（`logger.Info()` 等）には伝播しない。そのため、送信失敗の観測経路を以下のように定義する。

- **型付きヘルパーが `error` を返す**: `LogAlert(ctx, alert) error` 等のヘルパー関数は `Handle()` の戻り値をラップして呼び出し元へ返す。業務処理側は必要に応じてこの `error` を確認できる
- **Slack ハンドラ内部からのフォールバックログ**: `Handle()` が HTTP 送信に最終失敗した場合、その詳細を Debug Logger（stderr・ファイル）に記録する。slog 呼び出し元へのエラー伝播は行わない

**通知処理は同期**:

`slog.Handler.Handle()` 内でリトライを含む HTTP 送信を同期的に行う。最悪ケースで最大約 34 秒（F-007 参照）呼び出しスレッドをブロックしうる。IMAP ポーリングとの非同期化はアーキテクチャ設計（タスク 02）で判断する。

**受け入れ条件（Acceptance Criteria）**:

- `AC-01`: `slog.Handler` インターフェースを実装した Slack ハンドラを提供する
- `AC-02`: INFO レベルのログレコードは正常時 Webhook URL に送信する
- `AC-03`: WARN・ERROR レベルのログレコードは異常時 Webhook URL に送信する
- `AC-04`: HTTP リクエストが最終的に失敗した場合（リトライ上限到達）、`Handle()` はエラーを返し、Debug Logger にエラー詳細を記録する
- `AC-05`: Slack API が回復不能なエラーレスポンス（リトライ不対象の 4xx）を返した場合は即座にエラーを返し、Debug Logger に記録する
- `AC-05a`: 型付きヘルパー関数（`LogAlert` 等）は `Handle()` の戻り値をラップして呼び出し元へ返す
...
Slack attachment を用いて、ログレベルや通知種別を色・絵文字で視覚的に区別する。これにより、チャンネルを流れるメッセージの優先度が一目でわかるようにする。

| ログレベル / 通知種別 | attachment color | 絵文字 |
|---|---|---|
| INFO（週次サマリ・正常） | `good`（緑） | ✅ |
| WARN（TLS failure・パースエラー等） | `warning`（橙） | ⚠️ |
| ERROR（IMAP 認証失敗・接続断等） | `danger`（赤） | 🚨 |

attachment の `fields` に構造化された情報を格納し、以下の項目を個別フィールドとして表示する（F-005 の AC-17〜AC-20 の各フィールドも同様に構造化する）。

**メッセージ長**:

通知メッセージが上限長を超えた場合、可読性を維持するため切り詰めを行う。
...
実行（プロセス起動）ごとに一意な識別子（Run ID）を生成し、すべての通知メッセージの attachment fields に含める。これにより、即時アラートと週次サマリ、Slack 上の通知とサーバ側のログを相関できる。

**受け入れ条件（Acceptance Criteria）**:

- `AC-17`: 通知メッセージに送信元組織名（`organization-name`）が含まれる
- `AC-18`: 通知メッセージに対象ポリシータイプ（`sts` / `tlsa`）が含まれる
- `AC-19`: 通知メッセージに failure_session_count の値が含まれる
- `AC-20`: 通知メッセージにレポートの対象期間（`date-range`）が含まれる
- `AC-20a`: 通知メッセージに Run ID（プロセス起動ごとに一意な識別子）が含まれる
- `AC-20b`: メッセージ本文が 4000 文字を超える場合、末尾を切り詰め、切り詰めを示すサフィックス（例：`...`）を付与する
- `AC-20c`: 個別フィールド値が 1000 文字を超える場合、同様に切り詰めを行う
- `AC-20d`: INFO レベルの通知は attachment color `good`（緑）と絵文字 ✅ を使用する
- `AC-20e`: WARN レベルの通知は attachment color `warning`（橙）と絵文字 ⚠️ を使用する
- `AC-20f`: ERROR レベルの通知は attachment color `danger`（赤）と絵文字 🚨 を使用する
- `AC-20g`: 通知メッセージのフィールド情報（組織名・ポリシータイプ・failure カウント等）を Slack attachment の `fields` として構造化して送信する
...
- メッセージフォーマットの単体テスト（F-005 の各フィールド・Run ID・truncation・color・emoji・attachment fields 含む）
- 送信失敗の観測テスト（AC-04/AC-05/AC-05a）:
  - 全リトライ失敗時に `Handle()` がエラーを返すこと
  - 型付きヘルパー（`LogAlert` 等）が `Handle()` のエラーを呼び出し元へ伝播すること
  - 失敗詳細が Debug Logger に記録されること（slog 呼び出し元へは伝播しないこと）
- ログレベルと送信先の振り分けテスト:
