...
どちらも `Flush()` 時に error webhook へ送信する。同一ポーリング実行内で両方が発生した場合は、それぞれ独立したメッセージとして送信する。
...
- `AC-20c2`: ファイルログ（Debug Logger 経由）への出力には切り詰めを適用せず、通知イベントの全内容を記録する
...
- `AC-20g`: INFO レベルの通知（定期サマリ）は attachment color `good`（緑）と絵文字 ✅ を使用する
- `AC-20h`: 各通知エントリのフィールド情報を Slack attachment の `fields` として構造化して送信する
- `AC-20i`: システムエラーアラートのタイトル行にエラー種別が含まれる
- `AC-20j`: システムエラーアラートにエラーメッセージが含まれる
- `AC-20k`: システムエラーアラートに発生コンポーネント名が含まれる
- `AC-20l`: TLS failure アラートとシステムエラーアラートが同一 `Flush()` 内で発生した場合、それぞれ独立したメッセージとして送信される
...
- TLS failure 集約メッセージのフォーマットテスト（`AC-17`〜`AC-20h`）:
  ...
  - ファイルログ出力には同じイベントが切り詰めなしで記録されること（`AC-20c2`）
- システムエラーアラートのフォーマットテスト（`AC-20i`〜`AC-20l`）:
  - タイトル行にエラー種別が含まれること
