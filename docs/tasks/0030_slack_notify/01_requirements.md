...
- `AC-16b`: 定期サマリの集計対象期間は TOML 設定ファイル（仮称 `summary.window_days`、デフォルト 7 日）および `summary --since` フラグで変更可能とする（タスク 0060 AC-15 / タスク 0070 AC-07a 参照）。**送信タイミングの制御は外部スケジューラー（cron / systemd timer）およびエントリポイントの責務**であり、本パッケージ（`internal/notify`）は送信間隔を管理しない
