...
- **即時アラート**: 以下のいずれかが発生した際に管理者へ迅速に伝える
  - TLS 接続失敗の検出（TLSRPT レポートで `failure_session_count > 0`）
  - 本システム自体の障害（IMAP 認証失敗・接続断・レポートのパースエラー・ストレージ障害等）
- **定期サマリ（集計対象期間は呼び出し側で指定可能、デフォルト：7 日）**: TLS 接続失敗ゼロの期間も含め、定期的に通知を送ることでシステムが正常稼働していることを確認する。通知が途絶えた場合は本システム自体の障害として検知できる。送信タイミングは外部スケジューラーが制御する
...
- メール通知（将来の拡張）
- 定期サマリの集計ロジック（タスク 0050 で担当、本タスクのハンドラを利用）
- 定期サマリの送信タイミング制御（外部スケジューラー cron/systemd timer が担当。タスク 0070 AC-07a / 0060 AC-15 参照）
- 通知の重複制御・レート制限
- Webhook URL の暗号化保存
- ログファイルのローテーションおよび古いファイルの削除（logrotate 等による管理はユーザー向けドキュメントで扱う）
...
- `Handle()` はバッファに積む: `slog.Handler.Handle()` は即座には HTTP 送信せず、内部バッファにレコードを蓄積する。この操作は I/O を伴わないため低遅延であり、呼び出し元を長時間ブロックしない
...
| イベント | ログレベル | 送信先 |
|---|---|---|
| 定期サマリ（TLS 失敗なし） | INFO | success webhook |
...
| 定期サマリ作成時のデータ読み出し失敗 | ERROR | error webhook |
...
- `AC-16b`: 定期サマリの集計対象期間は TOML 設定ファイル（仮称 `summary.window_days`、デフォルト 7 日）および `summary --since` フラグで変更可能とする（タスク 0060 AC-15 / タスク 0070 AC-07a 参照）。**送信タイミングの制御は外部スケジューラー（cron / systemd timer）およびエントリポイントの責務**であり、本パッケージ（`internal/notify`）は送信間隔を管理しない。タスク `0050` は集計ロジックのみを担う

### F-005: 即時アラートのメッセージフォーマット

即時アラートには 2 種類ある。

1. **TLS failure アラート**（WARN）: TLSRPT レポートで `failure_session_count > 0` が検出された場合
2. **システムエラーアラート**（ERROR）: 本ソフトウェア自体の障害（IMAP 認証失敗・接続断・パースエラー・ストレージ障害等）

どちらも `Flush()` 時に error webhook へ送信する。同一ポーリング実行内で両方が発生した場合は、それぞれ独立したメッセージ（複数の HTTP リクエスト）として逐次送信する。
...
**メッセージの構造**（システムエラーアラート）:

システムエラーは 1 イベント 1 メッセージとして送信する（集約しない）。

```
タイトル行（text）: 🚨 System Error: <エラー種別>
attachment（color: danger）:
  fields:
    - Error:     <エラーメッセージ>
    - Component: <発生コンポーネント（例: imap, storage）>
    - Run ID:    xxxxxxxx
```
...
ログファイルのローテーションや古いファイルの削除（logrotate 等による管理）は本タスクの対象外とする。将来ユーザー向けドキュメントを整備する際には、logrotate や systemd の設定例を含めることが望ましい。

**Run ID**:

実行（プロセス起動）ごとに一意な識別子（Run ID）を生成し、すべての通知メッセージの attachment fields に含める。これにより、集約アラート・定期サマリ・サーバ側ログを相関できる。
...
- `AC-20d`: ファイルログ（Debug Logger 経由）への出力には切り詰めを適用せず、通知イベントの全内容を記録する
- `AC-20e`: TLS failure 集約メッセージはタイトル行に影響組織数（N）を含む
- `AC-20f`: WARN レベルの集約メッセージは attachment color `warning`（橙）と絵文字 ⚠️ を使用する
- `AC-20g`: ERROR レベルの通知は attachment color `danger`（赤）と絵文字 🚨 を使用する
- `AC-20h`: INFO レベルの通知（定期サマリ）は attachment color `good`（緑）と絵文字 ✅ を使用する
- `AC-20i`: 各通知エントリのフィールド情報を Slack attachment の `fields` として構造化して送信する
- `AC-20j`: システムエラーアラートのタイトル行にエラー種別が含まれる
- `AC-20k`: システムエラーアラートにエラーメッセージが含まれる
- `AC-20l`: システムエラーアラートに発生コンポーネント名が含まれる
- `AC-20m`: TLS failure アラートとシステムエラーアラートが同一 `Flush()` 内で発生した場合、それぞれ独立したメッセージとして逐次送信される
...
- `AC-28`: HTTP 5xx または 429 が返された場合、最大 3 回リトライする。429 の場合は `Retry-After` ヘッダーが存在すればその値を待機時間として優先し、存在しない場合は指数バックオフ（基準値 2 秒）を適用する
...
- `AC-38`: dry-run モード時は HTTP リクエストを発行せず、送信予定のメッセージを Debug Logger（stderr）に出力する。これにより、Webhook URL を設定せずに端末で通知内容を確認できる（stdout 通知ハンドラの代替手段）
...
専用の「stdout 通知ハンドラ」は実装しない。単体テストおよび結合テストでは **スパイハンドラ**（`SpyNotifier` 等）を使用し、`Flush()` が生成するメッセージペイロードを検査する。go-safe-cmd-runner の `newSlackHandlerFunc` 変数差し替えパターンを参考に、ハンドラ生成関数を変数化してテスト時にモックに切り替える。

手動検証時（Webhook URL を設定せずに動作を確認したい場合）は `--dry-run` フラグを使用する。`Flush()` が HTTP POST をスキップし、送信予定のメッセージを Debug Logger（stderr）に出力するため、通知内容を端末で確認できる。
...
- TLS failure 集約メッセージのフォーマットテスト（`AC-17`〜`AC-20h`）:
  - 複数 failure が 1 メッセージに集約されること
  - タイトル行に影響組織数 N が含まれること
  - Slack 送信ペイロードは上限長で切り詰められること（`AC-20b`/`AC-20c`）
  - ファイルログ出力には同じイベントが切り詰めなしで記録されること（`AC-20d`）
- システムエラーアラートのフォーマットテスト（`AC-20j`〜`AC-20m`）:
  - タイトル行にエラー種別が含まれること
  - エラーメッセージ・コンポーネント名・Run ID が `fields` に含まれること
  - TLS failure と同一 `Flush()` 内で発生した場合、独立したメッセージとして送信されること
