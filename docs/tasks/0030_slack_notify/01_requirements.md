...
- `AC-04`: HTTP リクエストが失敗した場合（タイムアウト、接続エラー等）はエラーを返す
- `AC-05`: Slack API がエラーレスポンス（4xx, 5xx）を返した場合はエラーを返す
...
- `AC-26a`: TOML 設定ファイルに Webhook URL を示すキー（例：`webhook_url`、`url`、`webhook_url_success`、`webhook_url_error` 等、`notify.slack` セクション配下の URL を含む可能性のあるキー）が存在する場合、起動時に設定エラーを返す（fail-fast）。これは機密情報がバージョン管理されることを防ぐためのセキュリティ対策である。エラーメッセージには「Webhook URL は環境変数 `TLSRPT_SLACK_WEBHOOK_URL_SUCCESS` / `TLSRPT_SLACK_WEBHOOK_URL_ERROR` で設定すること」と表示する
...
- `AC-40`: dry-run モードの有効化は CLI フラグまたは設定ファイルから制御できる（具体的な設定方法はアーキテクチャ設計で定める）
...
- リトライ最大 4 回（初回 + 3 回再試行）と指数バックオフ（基準値 2 秒）により、最悪ケースの送信所要時間は約 `5s × 4 + (2s + 4s + 8s) = 34 秒` を上限とする
- 上記の所要時間中に IMAP ポーリング処理がブロックされないよう、Slack 通知は呼び出し元の処理から独立して動作できることが望ましい（具体方法はアーキテクチャ設計で定める）
...
- URL 検証テスト（F-006）:
  ...
  - TOML 設定ファイルに Webhook URL 系のキーが存在する場合に fail-fast すること
- HTTP リトライテスト（F-007）:
