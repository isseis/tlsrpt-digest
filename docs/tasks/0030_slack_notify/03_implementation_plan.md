...
**備考（`AC-16b`）**: 送信タイミングの制御は外部スケジューラー（cron / systemd timer）とエントリポイント（タスク `0070` AC-07a）の責務であり、本タスクでも `0050` でも実装しない。タスク `0050` は集計ロジック（集計期間を引数で受け取る API）を提供する。本タスクでは `Summary` が任意期間を受け取って通知できることをもって、通知パッケージ側の受け入れ条件を満たす。
