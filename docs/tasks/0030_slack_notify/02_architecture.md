...
8. **定期サマリとの責務境界**: 定期サマリの集計はタスク `0050` が担い、本タスクは集計済みサマリの通知表現と配送のみを担う。送信タイミングの制御は外部スケジューラー（cron / systemd timer）およびエントリポイント側の責務であり、`internal/notify` は間隔を仮定しない
...
| タスク `0050` の定期サマリ処理 | 正常レポートの集計（集計ロジック） | 本タスクでは再実装せず、集計済みデータを受け取るだけとする |
...
| `internal/notify/validate.go` | `ValidateEnvCombination`（エクスポート）、`validateWebhookURL`/`validateBothURLs`（非エクスポート）、`BuildHandlers`（エクスポート、`main.go` からのエントリポイント） | **新規** |
| `internal/notify/errors.go` | エラー型定義（`WebhookValidationError`、`SlackServerError`、`SlackClientError`） | **新規** |
| `internal/notify/testutil/mocks.go` | テスト用スパイハンドラ（`SpyHandler`、`//go:build test`、`package notifytestutil`） | **新規** |
...
| テスト内容 | 使用ツール |
|---|---|
| `httptest.NewTLSServer` によるモック Slack サーバへの送信検証（`SlackHandlerOptions.HTTPClient` に自己署名証明書を信頼する `*http.Client` を注入） | `net/http/httptest` |
| success / error で別 TLS サーバを用いた振り分け検証 | `net/http/httptest` |
...
10. `internal/notify/testutil/mocks.go` — スパイハンドラ（`//go:build test`）
...
| 定期サマリ間隔の変更 | 間隔制御は外部スケジューラー（cron / systemd timer）とエントリポイント側の責務であり、本パッケージへの変更は不要 |
