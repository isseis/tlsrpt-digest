...
`report_end_date` は TLSRPT レポート送信者が設定する値であり、遠未来日付（例：year 3000）の誤設定や意図的な攻撃に対して脆弱である。GC の目的は「古いレポートデータを削除する」ことであり、データの古さはダウンロード日時（`saved_at`）ではなく IMAP サーバー受信日時（`INTERNALDATE`、先方の送信日時の近似値）で計測するのが意味的に正しい。
