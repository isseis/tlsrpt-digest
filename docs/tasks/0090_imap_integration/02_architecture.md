...
- **テスト専用設定はテストコードから注入する**: greenmail への接続に必要な `InsecureSkipVerify: true` は、統合テストコードが `imap.Config` を組み立てる際にのみ設定する。recovery E2E テストでは `fetchRunner.newMailFetcher` の差し替え（依存注入）を通じて注入する。
...
flowchart TD
    ...
    ENV[("環境変数<br>IMAP_TEST_*")] --> IT["統合テスト<br>(build tag: integration)"]
    IT --> CLIENT["imapClient<br>(変更なし)"]
    IT --> HELPER["テストヘルパー"]
    CLIENT -->|"IMAPS 3993"| GM[("greenmail<br>インメモリ IMAP/SMTP サーバ")]
...
graph TB
    ...
    subgraph imap_pkg ["internal/imap/"]
        CFG["Config<br>(InsecureSkipVerify 追加)"]
        TLS["buildTLSConfig<br>(フィールド反映)"]
        CLIENT["imapClient<br>(変更なし)"]
        ITEST["client_integration_test.go<br>(テスト拡張)"]
    end

    subgraph cmd_pkg ["cmd/tlsrpt-digest/"]
        FR["fetchRunner<br>(変更なし)"]
        RR["recoverRunner<br>(変更なし)"]
        RTEST["recovery_integration_test.go<br>(新規)"]
    end
...
flowchart TB
    ...
    subgraph devcontainer ["devcontainer (Docker Compose)"]
        DEV["dev-env コンテナ<br>go test"] -->|"greenmail:3993 / 3025<br>(コンテナ間ネットワーク)"| GMD[("greenmail コンテナ")]
    end

    subgraph actions ["GitHub Actions (ubuntu-latest)"]
        JOB["integration-test ジョブ<br>go test"] -->|"localhost:3993 / 3025<br>(公開ポート)"| GMA[("greenmail<br>service container")]
    end
...
| 受信者アドレス導出 | テスト名から一意かつ有効なメールアドレスを生成する | テスト名のサニタイズ |
...
本機能のセキュリティ上の主要な懸念は「TLS 証明書検証を無効化する `InsecureSkipVerify` が本番経路に混入すること」である。通知の送信・宛先の取り扱いは行わないため、通知セキュリティガイドラインは対象外。
...
- `InsecureSkipVerify: true` の設定はテストコード（`//go:build integration`）からのみ行う。
...
flowchart TD
    ...
    FIELD["InsecureSkipVerify<br>(imap.Config フィールド)"]
    THREAT["脅威: 本番接続で<br>証明書検証が無効化される"]
    M1["緩和1: config.IMAPConfig /<br>TOML に公開しない"]
    M2["緩和2: buildIMAPConfig が<br>設定しない (AC-03 で検証)"]
    M3["緩和3: 有効化は integration<br>タグのテストコードのみ"]
...
greenmail はテスト専用のインメモリサーバであり、ユーザのパスワードはメールアドレスと同一という単純な規約に従う。これらはテスト環境（devcontainer・CI service container）に閉じた値であり、本番の秘密情報とは無関係である。接続情報は環境変数から取得し、テストコードにハードコードしない。
...
AC-05（空メールボックス）の固定ユーザは、greenmail の起動時オプションで事前登録しておく。SMTP 配送でユーザを作成すると INBOX にメッセージが残り「空である」前提が崩れるため、配送による作成には依存しない。
...
sequenceDiagram
    ...
    participant R as runCLI (fetch/recover)
    participant ST as store (実ファイル)
    participant GM as greenmail

    T->>A: 固有名の非 INBOX メールボックスを CREATE
    A->>GM: CREATE mailbox
    T->>R: fetch 実行 (InsecureSkipVerify 注入)
...
flowchart TD
    ...
    PR["PR の変更ファイル"] --> CHK{"Go ソース / Makefile /<br>workflow / .devcontainer /<br>testdata のいずれかを含む?"}
    CHK -->|"Yes"| RUN["integration-test を実行"]
    CHK -->|"No"| SKIP["integration-test をスキップ"]
