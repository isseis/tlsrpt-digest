...
**フェーズ完了の確認**:
- [x] greenmail IMAPS 環境（Phase 5 の devcontainer 更新後、または同等の手動設定）で `go test -v -count=1 -tags test,integration ./internal/imap/...` が通過すること
...
- [x] PR で `.github/workflows/ci.yml` の変更を含む変更を push し、GitHub Actions の `integration-test` ジョブが起動することを確認する
