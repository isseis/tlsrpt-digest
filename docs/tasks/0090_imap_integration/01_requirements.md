...
| レビュー日 | - |
| レビュアー | - |
| コメント | 実装コードとの突き合わせレビューを実施。AC-10 のエラー表現、SMTP 注入とメールボックス分離の整合性、CREATE/DELETE が抽象外である点などを修正。 |
...
- devcontainer の IMAP テスト接続ポートを IMAPS（3993）に変更（greenmail は `setup.test.all` により 3993/IMAPS を既に待ち受けているため、`dev-env` からの接続は環境変数 `IMAP_TEST_PORT` の変更で足りる。現行の 3143/平文ポートは TLS 専用の `imapClient`（`NewIMAPClient` は常に TLS でダイヤルする）では利用できない）
...
- **AC-03**: 統合テストの実行に必要な環境変数（最低限 IMAP_TEST_HOST、IMAP_TEST_PORT。SMTP 注入を伴うテストは IMAP_TEST_SMTP_HOST、IMAP_TEST_SMTP_PORT も）が設定されていない場合、統合テストは `t.Skip()` でスキップされ、CI の通常テストパスを妨げない
...
- **Makefile（任意）**: `go test -tags integration ./...` を実行する `make test-integration` 等のターゲット追加は本要件のスコープ外だが、ローカル実行の利便性向上のため別途検討してよい。
