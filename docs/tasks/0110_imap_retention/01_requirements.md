...
- `AC-02`: `retention_days` 未指定時のデフォルトは `0`（無効）とする。リモート削除は不可逆であるため、デフォルトでは無効とし、利用者が明示的に正の値を設定したときのみ有効化する（オプトイン）。これによりアップグレード時の既存環境への影響（意図しない自動削除・IMAP 認証情報を持たない既存 `gc` ジョブの失敗）を回避する。
...
- `AC-13`: IMAP 操作で発生したエラーは、ローカルストアのエラーと混同しないよう、既存の IMAP 専用 `SystemErrorKind`（`SystemErrorKindIMAPConnectFailed` / `SystemErrorKindIMAPAuthFailed` / `SystemErrorKindIMAPOperationFailed`）にマッピングして `notifyGCSystemError` で通知する（`gcNotifyKind` のローカルストア用デフォルト `SystemErrorKindStorePermission` に巻き込まれないようにする）。
...
- IMAP 削除はデフォルト無効（`retention_days = 0`）であり、利用者が明示的に正の値を設定したときのみ有効化される（オプトイン）。有効化した場合の以下の挙動を README に明記する。
  - 設定した保持期間を超えた IMAP 上のメールが `gc` 実行時に自動削除される。
  - 有効時は `gc` が IMAP 認証情報を必要とする（デフォルト・無効時は不要）。
...
- config: デフォルト 0（無効）（AC-02）、`0` で無効（AC-03）、負値でエラー（AC-04）、不変条件違反でエラー（AC-05）、`fetch_days` / `window_days` 境界値、エラー型を `errors.Is` で検証（AC-06）。
