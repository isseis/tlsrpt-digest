...
1. 有効な `.json.gz`（gzip 圧縮）バイト列を入力すると、展開された JSON バイト列を返す
2. 有効な非圧縮 JSON バイト列を入力すると、そのまま JSON バイト列として扱う
3. 不正な gzip データを入力するとエラーを返す
4. 展開後のデータが有効な JSON でない場合もエラーを返す
5. gzip・非圧縮ともに、展開（またはそのまま）のサイズが上限を超える場合はエラーを返す
