...
- **直接変更**: `internal/notify/`（以下の3ファイルを変更する）
  ...
  - `helpers.go`: `LogSummary` を更新し、`OrganizationStats` の各エントリを slog 属性として出力する
...
Slack Incoming Webhook は 1 attachment あたり最大 10 フィールドまでという制約がある。組織数が多い場合にこの制限を超える可能性があるため、`formatSummary` は `formatAlerts` と同様に attachment をチャンクして複数に分割する実装とする（1 attachment あたり最大 9 組織フィールド、最後の attachment にのみ Run ID フィールドを追加）。
...
- `AC-10`: 組織数が 9 を超える場合、組織フィールドは複数の attachment に分割される（1 attachment あたり最大 9 フィールド。Run ID は最後の attachment にのみ付与）
