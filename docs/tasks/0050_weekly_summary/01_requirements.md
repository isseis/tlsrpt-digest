...
レポート取得は `store.GetAllReports()` を使用する。開始・終了日時によるフィルタリング（`start <= report.DateRange.EndDatetime <= end`）および failure 除外（`report.HasFailure() == true`）はすべてアプリケーション層で実施する。store 層はフィルタリングを担わない。

**受け入れ条件（Acceptance Criteria）**:

- `AC-01`: 集計期間（開始日時・終了日時）を引数で受け取り、`DateRange.EndDatetime` が集計期間内に収まり、かつ `HasFailure() == false` のレポートを組織名別にグループ化して集計できる
