...
sequenceDiagram
    ...
    SH->>TR: text / fallback / fields を制限内へ切り詰め
...
Block Kit 用の `Blocks` / `slackBlock` 型は YAGNI 原則に従って持たない。即時アラートは `attachment.fields` と `attachment.fallback` だけを使い、アラート以外（警告・システムエラー・サマリー）の `fields` ベース整形も従来どおり維持する。
...
| `internal/notify/message.go` | `slackAttachment` に `Fallback` を追加。未使用の `Blocks` / `slackBlock` 型は削除 |
| `internal/notify/helpers.go` | `LogAlert` で新フィールドを slog 属性へエンコード |
| `internal/notify/format.go` | `extractAlert` で新フィールドをデコード。`formatAlerts` は warning attachment fields と fallback を生成。`truncateMessage` は top-level text、attachment fallback、fields を切り詰める |
...
3. **サイズ制限**: policy field 表示上限、failure detail 表示上限、fallback / fields の切り詰めを実装する。
