...
| 最終更新日 | 2026-06-09 |
| レビュアー | isseis |
| コメント | 実 Slack 表示確認に基づき、即時アラートの主表示を Block Kit sections ではなく warning attachment fields に修正 |
...
- **本文重複を避ける**: Slack は top-level `text` と attachment の両方を通常画面に表示するため、top-level `text` には短いタイトルだけを置く。本文相当の詳細は attachment fields に置く。
...
flowchart LR
    ...
    SLACK["Slack メッセージ<br>(text + attachment fields)"]
...
sequenceDiagram
    ...
    F->>FA: []Alert
    FA-->>SH: slackMessage(text + attachment fields)
    Note over SH: DebugLogger 有効時は切り詰め前の全文を出力
    SH->>TR: text / fields を制限内へ切り詰め
...
即時アラートは、top-level `text` にタイトルだけを置き、詳細本文を warning attachment fields に置く。
...
Block Kit 用の `Blocks` / `slackBlock` 型は YAGNI 原則に従って持たない。即時アラートは `attachment.fields` だけを使い、アラート以外（警告・システムエラー・サマリー）の `fields` ベース整形も従来どおり維持する。
...
外部由来文字列（組織名、Report ID、`result-type`、`receiving-mx-hostname`、`failure-reason-code`）は、制御文字を空白へ正規化したうえで field value に挿入する。表示は Markdown 装飾に依存しないプレーンな文字列で構成し、改行注入や偽の行見出しを作れないようにする。
...
そのため最終設計では、主表示を legacy attachment fields に戻す。通常の Slack 画面では重複せず旧来の黄色いブロック表示を維持する。
...
| `internal/notify/message.go` | `Blocks` / `slackBlock` 型を持たないシンプルな構造 |
| `internal/notify/helpers.go` | `LogAlert` で新フィールドを slog 属性へエンコード |
| `internal/notify/format.go` | `extractAlert` で新フィールドをデコード。`formatAlerts` は warning attachment fields を生成。`truncateMessage` は top-level text と fields を切り詰める |
| `cmd/tlsrpt-digest/notify_helpers.go` | `logAlerts` で `report.ReportID` と `policy.FailureDetails` の公開 4 項目だけを `Alert` に写像 |
| `internal/notify/*_test.go` | fields の JSON shape、本文非重複、切り詰め、機微情報非混入を検証 |
...
外部由来文字列は制御文字を空白へ正規化し、値ごとの上限で切り詰める。field の表示は Markdown 装飾に依存しないプレーンな文字列として構成し、改行注入や偽の行見出しを作れないようにする。
...
| Slack payload shape | アラートが title-only top-level text、warning attachment、fields を持つ |
| 表示回帰 | 旧来の `Organization / Policy / Failures / Period` field 表示、Report ID、Failure Details、Run ID が出る |
| 本文非重複 | top-level `Text` に詳細本文を含めず、詳細は attachment fields に置く |
...
| Slack smoke | `make test-slack-notify` で黄色い attachment 表示と fields の送信 payload を確認する |
...
2. **Slack 表示整形**: `formatAlerts` を warning attachment fields へ更新し、top-level `text` をタイトルのみにする。
3. **サイズ制限**: policy field 表示上限、failure detail 表示上限、fields の切り詰めを実装する。
4. **テスト更新**: payload shape、本文非重複、機微情報遮断、既存通知回帰を追加・更新する。
...
- 複数メッセージ分割やファイル添付は本タスクの範囲外とする。
- Block Kit を再導入する場合は、新しい具体要件が出た時点で改めて設計し、本文消失・本文重複・黄色い attachment 表示の維持を再検証する。現時点では YAGNI として型も保持しない。
