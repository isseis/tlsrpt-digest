...
### Phase 1a: 環境変数ヘルパーとユニットテスト（常時実行）
...
なお `t.Skip` を行うラッパー `loadSlackNotifyTestEnv` は、唯一の呼び出し元が統合テスト（`slack_notify` タグ側）であるため Phase 1b の `slack_notify` ファイルに置く。これを `test` タグ側に置くと、`make lint`（`golangci-lint run --build-tags test`、`slack_notify` を含まない）で呼び出し元ゼロとなり `unused` 違反になるため避ける。`test` タグ側に置く純粋ヘルパー `missingSlackNotifyEnv` は自身のユニットテストから呼ばれるため `unused` にならない。
...
### Phase 1b: 統合テストドライバの実装（手動実行）

**対象ファイル**: `cmd/tlsrpt-digest/slack_notify_integration_test.go`（新規、`//go:build slack_notify`、`package main`）

- [ ] スキップラッパー `loadSlackNotifyTestEnv(t *testing.T) string` を実装する。`missingSlackNotifyEnv(nil)`（Phase 1a で定義、`test,slack_notify` 併用時に可視）が非空なら `t.Skip` し、設定済みなら `os.Getenv(slackNotifyWebhookEnvKey)` の Webhook URL 文字列を返す。
...
| マイルストーン | 内容 | 成果物 |
|---|---|---|
| M1 | Phase 1a・1b 完了 | `slack_notify_env_test.go`（env ヘルパー＋ユニットテスト、`make test` で実行）と `slack_notify_integration_test.go`（統合テスト、コンパイル・lint 通過） |
| M2 | Phase 2 完了 | `Makefile` の `test-slack-notify` ターゲット |
| M3 | Phase 3 完了 | 実 Slack への送信成功と目視確認、永続ファイル非作成の確認 |

Phase 1a/1b は [`02_architecture.md`](02_architecture.md) §8 の step 1（テストドライバ実装）を、常時実行する env ヘルパーを先行させるために細分したものである。Phase 1b（統合テスト）は Phase 1a の純粋ヘルパー `missingSlackNotifyEnv` を前提とするため、1a→1b の順で実装する。Phase 2・3 はそれぞれ §8 の step 2・3 に対応する。
...
- [ ] Phase 1a: `slack_notify_env_test.go` 作成（env キー定数・`missingSlackNotifyEnv`・`TestSlackNotify_EnvRequirements`、`//go:build test`）
- [ ] Phase 1a: `make test` で `TestSlackNotify_EnvRequirements` が実行され pass する。`golangci-lint run --build-tags test` が 0 issues
- [ ] Phase 1b: `slack_notify_integration_test.go` 作成（`loadSlackNotifyTestEnv`・`TestSlackNotify_FailureAlert_Integration`、`//go:build slack_notify`）
- [ ] Phase 1b: コンパイル・`make fmt`・lint 通過
- [ ] Phase 2: `Makefile` に `test-slack-notify` 追加（`.PHONY` 更新含む）
- [ ] Phase 2: env 未設定での skip 動作確認
- [ ] Phase 3: 実 Webhook 送信・Slack 目視確認・`git status` クリーン確認
