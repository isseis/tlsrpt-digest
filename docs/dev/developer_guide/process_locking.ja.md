...
- `AbortReset` の restore 処理（sentinel を変更しないため guard 不要。ただし sentinel の
...
理由：reset 操作中（フェーズ 1〜5）にはデータファイルがステージングに移動されている
可能性がある。`withGuardExclusive` はセンチネルの可視性しか保証しないため、
pending reset を無視して `recovery_required` を消すと「new UIDValidity + cleared
recovery_required + データなし」という矛盾状態が生じる。

`ApplyRecovery` は manifest が存在する場合に `ErrPendingReset` を返すことで、
この経路を store 層で閉じている。**`recovery_required` を変更する新しい API を追加する
場合も同様に、pending reset 中の呼び出し可否を設計時に検討し、必要なら
...
- マニフェスト作成だけを summary guard で囲み、
  後続のステージング / commit / cleanup を無保護にする
...
- [ ] pending reset 中（manifest 存在時）に呼ばれた場合の挙動を設計している：
