...
`summary` 同士の並走は summary consistency guard（共有ロック）が許容するため問題ない。
...
ガードファイルが実際に存在することがガード機能の前提条件である。
...
すでに旧 inode に `LOCK_SH` を取得している `summary` と、新 inode に `LOCK_EX` を取得した
writer が互いの flock を見えなくなり、排他保護が機能しなくなる。
...
- `recovery_required` を変更しない処理を ガードで囲み、
  `summary` を不要にブロックする
- マニフェスト作成だけを ガードで囲み、
...
- [ ] `recovery_required` を変更しない処理をガードで囲んでいない
