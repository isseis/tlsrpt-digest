...
互いに並走できない。`summary` は store-wide process lock を取得せず summary consistency guard
（共有ロック）のみを取得するため、書き込み系サブコマンドと並走できる。

`summary` 同士はいずれも共有ロックを取得するだけなので、互いにブロックせず並走できる。
