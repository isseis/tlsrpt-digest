...
（`ResetForRecovery`）の進捗を管理する状態機械を、
単一 writer 前提で安全に操作できるようにする。

この状態機械は次の 3 ファイルで構成される。

- **リセットマニフェスト**: 復旧操作の進捗を `resetPhase`（1・4）として記録する台帳
...
- `gc`
- `reprocess`
- `recover`（`--mode keep-old` / `discard-old` のいずれも）

### 契約

1. ストアを開く（`store.Open(...)` 呼び出し）より前に取得する。
2. 処理完了まで保持する（異常終了パスを含む）。
3. `recover --mode discard-old --yes` はロック保持中に `OpenRecoverReset` を使う。
4. `ResetForRecovery` は単一 writer 前提で設計されているため、
...
理由：リセット操作中（フェーズ 1）にはデータファイルがステージングに移動されている
...
sequenceDiagram
    ...
    Note over Stg,Sen: 【前提】ResetForRecovery がフェーズ 1 で中断<br>tlsrpt.json・emails/ がステージングに移動済み<br>ストアルートにデータなし・マニフェスト残存
...
`recovery_required` が立っていれば、その後のオペレーター操作（`recover`）によってストアデータが変更される見込みである。集計結果が古くなっている可能性が高いため、通知して終了する。
...
- [ ] 処理完了まで（異常終了パスを含む）store-wide process lock を保持している
- [ ] `recover --mode discard-old --yes` は
  store-wide process lock 保持中に `OpenRecoverReset` を使っている
- [ ] `ResetForRecovery` を直接呼ぶ場合は store-wide process lock を保持している
...
- [ ] `summary` が古くなった「復旧不要」判断で送信しないことをテストしている
