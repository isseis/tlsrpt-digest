...
Secret 型はあくまで**第二の防御線**であり、原則1〜4の設計による制約が主たる防御である。
...
1. `Notifier.SendAlert()` が `Alert` 構造体のフィールドのみをメッセージに含めることを検証するテスト
2. Config 構造体の機密フィールド（`Secret` 型）が通知メッセージに含まれないことを検証するテスト
3. デバッグ出力先（`io.Writer`）が Notifier のハンドラと別の変数で管理されていることを検証するテスト
