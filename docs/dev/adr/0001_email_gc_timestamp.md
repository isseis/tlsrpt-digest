...
GC 基準に `SavedAt` ではなく `INTERNALDATE` を採用する理由：GC の目的は「古いレポートデータを削除する」ことであり、データの「古さ」は我々がダウンロードした日時（`SavedAt`）ではなく、先方が送信した日時（`INTERNALDATE`）で計測するのが意味的に正しい。`SavedAt` を基準にすると、ネットワーク障害等でフェッチが遅延した場合に実質的な保持期間が短縮される。リカバリー時に保持期間を超えた古いメールを再取得した場合は即 GC 対象となるが、これは正しい動作である（保持期間を超えたデータの再処理は不要）。
...
| `SavedAt` | 使用しない | ゼロ値フォールバックのみ | 使用しない |
