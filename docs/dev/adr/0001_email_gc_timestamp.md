...
- **パスの安定性が壊れる（最重要）**: `SentAt` はメールの `Date:` ヘッダーから導出されるため、同じメールを何度処理しても同一パスになる。`SavedAt` はダウンロード日時であるため、リカバリー等で同一メールを再取得した際に異なる月になり得る。その場合 `SaveEmail` は別パスへ書き込もうとし、同一メールの重複ファイルを作成する。`SentAt` ベースのパスはこの冪等性を保証している。
