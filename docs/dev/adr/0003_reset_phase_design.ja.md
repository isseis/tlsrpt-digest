...
```
1. マニフェストを読む
   ├─ 不在 → 残留ステージングを best-effort 削除して正常終了（※後述）
...
ステージングの内容は復元に使われることがない。したがって best-effort で削除して安全である。
...
| 残留ステージング（マニフェスト削除後にステージング削除が失敗） | なし | なし | ステージングを best-effort 削除して通常 Open |
...
| フェーズ管理の削減 | 現在の `resetPhase`・マニフェスト・ステージング・abort phase の多くを DB の atomic commit に置き換えられる |
...
`.eml` をファイルシステムに残したまま、sentinel・recovery_required・UIDVALIDITY・report メタデータを bbolt トランザクションで原子更新する設計が自然。現在のフェーズ管理（resetPhase・マニフェスト・ステージング）の多くを `db.Update` 1 回に置き換えられる。ただし `.eml` とメタデータの二相コミット問題は残るため、`.eml` の追加・削除を伴う操作には別途クラッシュセーフ設計が必要になる。
