...
flowchart TD
    ...
    subgraph PendingReset["コミット前 pending reset（フェーズ 1–3）"]
...
`rename(2)` は POSIX の保証する原子操作であるため、成功した場合のみファイルが移動している。チェックポイントをリネームの後に書くことで、クラッシュ後の再開時に以下の推論が成立する。
...
| 観点 | 内容 |
|---|---|
| クラッシュセーフティ | `BEGIN` / `COMMIT` で report・sentinel・recovery_required の更新を 1 トランザクションに閉じ込められる |
...
| 本用途への適合 | sentinel・manifest・recovery_required・report メタデータの原子更新には十分。集計・条件検索の要件がない現時点では SQL は不要 |
...
| `.eml` の扱い | 大きな BLOB を bbolt に格納すると次の問題が生じるため、`.eml` はファイルシステムに残し bbolt との二相コミット問題を受け入れる設計が現実的。① bbolt は全データを mmap するためページキャッシュが圧迫される。② `db.Update` は DB 全体に書き込みロックをかけるため、大きな BLOB の書き込み中に他の操作がブロックされる。③ bbolt には自動スペース回収がなく、`.eml` 削除後もファイルサイズは縮小されないため、GC のたびに `Compact` 相当の処理を実装する必要がある。④ ファイルシステムに置けば `cat` や `mutt` で直接確認・再送できるが、bbolt に入ると専用ツールや Go コードが必要になり手動復旧コストが上がる |
| migration の表現 | スキーマバージョン管理はアプリケーション側で設計する必要がある |

**本プロジェクトへの評価**

`.eml` をファイルシステムに残したまま、sentinel・recovery_required・UIDVALIDITY・report メタデータを bbolt トランザクションで原子更新する設計が自然。現在のフェーズ管理（resetPhase・manifest・staging）の多くを `db.Update` 1 回に置き換えられる。ただし `.eml` とメタデータの二相コミット問題は残るため、`.eml` の追加・削除を伴う操作には別途クラッシュセーフ設計が必要になる。
...
**SQLite 系**（`mattn/go-sqlite3` または `modernc.org/sqlite`）へ移行する場合は、report のみを部分的に DB 化せず、report・sentinel・recovery_required を同一 DB に集約する。`.eml` をファイルシステムに残すと DB とファイルの二相コミット問題が残り、現在のフェーズ管理と同種の複雑さが再発する。

**bbolt** へ移行する場合は、メタデータ（sentinel・recovery_required・UIDVALIDITY・report メタデータ）を bbolt に集約し、`.eml` はファイルシステムに残す設計が現実的。この場合、`.eml` の追加・削除を伴う操作（fetch での保存、gc での削除）には bbolt トランザクションとファイル操作の順序を明示的に設計する必要がある。
