...
1. IMAP メールボックスのポーリングによるレポートメールの取得
...
| `internal/store` | SQLite を用いたレポートデータの永続化、週次サマリ用の集計クエリ |
| `cmd/tlsrpt-digest` | 設定ファイル読み込み、各パッケージの初期化、ポーリングループの制御 |
