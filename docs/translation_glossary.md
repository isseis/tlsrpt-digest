...
| 保留リセット | pending reset | リセット操作開始後、コミット完了前の中間状態（フェーズ 1 のみ）。コード上では `ErrPendingReset` として参照される。`HasPendingReset()` はこの状態を検出するが、フェーズ 4（committed、コミット後クリーンアップ残滓）に対しては false を返す。 |
...
| 柔軟性 | flexibility | |
| フロー | flow | |
| フェーズ | phase | 実装計画のフェーズ区分 |
| フローチャート | flowchart | Mermaid フローチャートの文脈 |
| フォーマット | format | |
| 凡例 | legend | ダイアグラムの色・記号説明 |
...
| サブグラフ | subgraph | Mermaid subgraph の文脈 |
| サンプル | sample | |
| スコープ | scope | |
| スクリプト | script | |
| 検索 | search | |
...
| 状態遷移図 | state diagram | Mermaid stateDiagram-v2 の文脈 |
...
- **verification**: 整合性の検証（hash verification、integrity check）
