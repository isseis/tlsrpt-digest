...
- **Integration test re-run safety.** For tests that create persistent state in an external service (IMAP mailboxes, database rows, S3 objects), explicitly ask: "Does this test leave artifacts that break the next run?" If yes, plan a unique run-scoped identifier (timestamp + crypto/rand) and embed it in all created resource names (email addresses, mailbox names, message IDs).
