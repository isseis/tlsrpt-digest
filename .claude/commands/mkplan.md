...
- Record the findings as a `既存コード調査結果` section in `03_implementation_plan.md` (see step 6). For each relevant area, note: what already exists, what is missing, and what needs to change. Omit areas where the existing code requires no attention.
...
- Add a `既存コード調査結果` subsection under the implementation overview (§1), summarising the findings from step 5.
