...
- When removing multiple scattered code sites (e.g. several test functions), delete them one at a time with Edit using the exact text read from the file. Do not script bulk deletion (e.g. a brace-counting loop); nested literals make such heuristics over-consume adjacent code. After each removal, check IDE diagnostics for unintended breakage.
